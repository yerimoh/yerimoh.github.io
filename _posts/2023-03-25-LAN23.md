...
Figure 1에 표시된 information extraction pipeline을 사용하여 327만 개 이상의 materials science abstracts에 대한 정보 추출을 높은 정확도로 수행했다.       
8천만 개 이상의 재료 과학 관련 엔티티가 추출되었으며 각 요약은 데이터베이스 컬렉션에서 구조화된 문서로 표현됩니다.     
각 문서의 엔티티는 정규화되어(즉, 동일한 용어가 함께 그룹화됨) 문서 검색에서 관련 결과의 수가 크게 증가합니다. 이러한 대규모 텍스트 마이닝은 재료 과학의 정보에 접근할 수 있는 엄청난 기회를 제공합니다.
