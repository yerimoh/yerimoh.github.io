...
**[base model의 특징]**      
* **목표**    
는 재료 과학 지식이 인코딩되는 방식으로 모델을 훈련하는 것입니다. 예를 들어, "알루미나"와 "SrTiO3"라는 단어가 재료를 나타내는 반면 "퍼터링"과 "MOCVD"는 합성 방법에 해당한다는 것을 컴퓨터에 가르치고 싶습니다. (i) 단어 표현, (ii) 로컬(문장 내) 컨텍스트, (iii) 단어 모양 등 특정 엔터티 유형에 해당하는 단어를 인식하도록 기계에 가르치는 데 사용할 수 있는 세 가지 주요 정보 유형이 있습니다.
