...
**이 프로젝트와 관련된 모든 데이터:**    
* NER의 train 데이터로 사용된 800개의 hand-annotated abstracts,     20
* entity 정규화를 위한 정보가 포함된 JSON 파일    21  
* 전체 entity 데이터베이스 327만 개는 온라인에서 JSON 형식으로 대량 다운로드할 수 있다 22
...
we have performed information extraction on over 3.27 million
materials science abstracts with a high degree of accuracy. Over
...
normalized (i.e., equivalent terms grouped together), greatly
increasing the number of relevant results in document retrieval.
Text mining on such a massive scale provides tremendous
opportunity for accessing information in materials science.



그림 1에 표시된 정보 추출 파이프라인을 사용하여 327만 개 이상의 재료 과학 추상에 대한 정보 추출을 높은 정확도로 수행했습니다. 8천만 개 이상의 재료 과학 관련 엔티티가 추출되었으며 각 요약은 데이터베이스 컬렉션에서 구조화된 문서로 표현됩니다. 각 문서의 엔티티는 정규화되어(즉, 동일한 용어가 함께 그룹화됨) 문서 검색에서 관련 결과의 수가 크게 증가합니다. 이러한 대규모 텍스트 마이닝은 재료 과학의 정보에 접근할 수 있는 엄청난 기회를 제공합니다.

텍스트 마이닝의 가장 강력한 응용 프로그램 중 하나는 정보 검색입니다. 정보 검색은 주어진 쿼리를 기반으로 정확하게 올바른 문서를 검색하는 프로세스입니다. 생물의학 분야에서, 정보를 지식 기반으로 분류하고 구성하는 것은 정보 검색의 효율성을 크게 향상시키는 것으로 밝혀졌습니다.54,55 출판된 문헌의 대규모 데이터베이스와 온톨로지를 결합하면 연구자들은 질의에 특정 의미론적 품질을 부여할 수 있습니다, 도메인에 구애받지 않는 단순한 텍스트 기반 검색보다 훨씬 강력합니다. 대상 쿼리가 원래 연구 기사에 연결될 수 있도록 각 추상에 대한 디지털 객체 식별자(DOI)를 말뭉치에 저장했습니다. 정보 검색 외에도, 이 규모의 텍스트 마이닝에서 훨씬 더 유망하고 새로운 측면은 섹션 4.2와 3.3에서 설명한 바와 같이 문헌의 "메타 질문"을 할 수 있는 잠재력입니다. 각 문서의 추출된 엔티티를 연결하고 수천 개의 문서에 대한 분석을 기반으로 요약 수준의 정보를 제공할 수 있는 잠재력은 연구자와 출판된 문헌 간의 상호 작용의 효율성을 크게 향상시킬 수 있습니다


One of the most powerful applications of text mining is in
information retrieval, which is the process of accurately
retrieving the correct documents based on a given query. In
the field of biomedicine, the categorization and organization of
information into a knowledge base has been found to greatly
improve efficiency in information retrieval.54,55 Combining
ontologies with large databases of published literature allows
researchers to impart a certain semantic quality onto queries,
which is far more powerful than simple text-based searches that
are agnostic to the domain. We have stored the digital object
identifier (DOI) for each abstract in our corpus, such that
targeted queries can be linked to the original research articles.
In addition to information retrieval, an even more promising
and novel aspect of text mining on this scale is the potential to
ask “meta-questions” of the literature, as outlined in sections
4.2 and 3.3. The potential to connect the extracted entities of
each document and provide summary-level information based
on an analysis of many thousands of documents can greatly
improve the efficiency of the interaction between researchers
and the published literature




We should note some limitations of the present information
extraction system, and the prospects for improvement. First,
certain issues arise as a result of only analyzing the abstracts of
publications. Not all information about a publication is
presented in the abstract; therefore, by considering only the
abstract, the recall in document retrieval can be reduced.
Second, the quantitative analysis of the literature, as in Figure
6, can be affected. This can be overcome by applying our NER
system to the full texts of journal articles. The same tools
developed here can be applied to the full text, provided that the
full text is freely available. A second issue with the current
methodology is that the system is based on co-occurrence of
entities, and we do not perform explicit entity relation
extraction. By considering only co-occurrence, recall is not
...
우리는 현재 정보 추출 시스템의 몇 가지 한계와 개선 전망에 주목해야 합니다. 첫째, 출판물의 요약만 분석한 결과 특정 문제가 발생합니다. 등록에 대한 모든 정보가 요약에 표시되지는 않으므로, 요약만 고려하면 문서 검색 시 리콜을 줄일 수 있습니다. 둘째, 그림 6과 같이 문헌의 양적 분석이 영향을 받을 수 있습니다. 이것은 저널 기사의 전체 텍스트에 NER 시스템을 적용함으로써 극복할 수 있습니다. 전체 텍스트를 자유롭게 사용할 수 있는 경우 여기서 개발한 동일한 도구를 전체 텍스트에 적용할 수 있습니다. 현재 방법론의 두 번째 문제는 시스템이 엔티티의 동시 발생을 기반으로 하며 명시적인 엔티티 관계 추출을 수행하지 않는다는 것입니다. 동시 발생만 고려하면 리콜은 영향을 받지 않지만 정밀도는 감소할 수 있습니다. 우리는 재료 과학 문헌을 쿼리하는 데 사용할 수 있는 다른 모든 도구가 동시 발생에 의존한다는 것에 주목합니다.

이러한 한계에도 불구하고, 우리는 대규모 텍스트 마이닝이 연구 도구를 개발하는 데 사용될 수 있는 방법을 입증했으며, 현재 작업은 출판된 자료 과학 문헌에 대한 인공 지능 및 프로그램 접근을 위한 첫 단계를 나타냅니다



Despite these limitations, we have demonstrated the ways in
which large-scale text mining can be used to develop research
tools, and the present work represents a first step toward
artificially intelligent and programmatic access to the published
