...
* JNLPBA에 대한 SOTA 모델은 JNLPBA뿐만 아니라 여러 NER 데이터 세트에 대해 훈련된 BiLSTM-CRF 앙상블이다(Yoon et al., 2018).       
* NCBI 질병의 SOTA 모델은 바이오버트(Lee et al., 2019)로, 생물의학 논문의 18B 토큰에 미세 조정된 BERTBase이다.      
