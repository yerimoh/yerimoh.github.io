...
동결된 임베딩 위의 작업별 아키텍처(+SCIBERT 사용 시 +3.25F1, BERT-Base 사용 시 +3.58)보다는 BERT 미세 조정을 통해 개선된 결과를 관찰한다. 각 과학 영역에 대해, 우리는 컴퓨터 과학(SCIBERT 사용 +5.59 F1, BERT-Base 사용 +3.17 F1)과 생물 의학 작업(+2.94 F1, BERT-Base 사용 +4.61 F1)에 대한 미세 조정의 가장 큰 영향을 관찰하고 다중 도메인 작업(+0.7 F1, BERT-BERT 사용 +1.14 F1)에 대한 가장 작은 영향을 관찰한다. BC5CDR 및 SciCite를 제외한 모든 데이터 세트에서 미세 조정이 적용된 BERT-Base는 동결된 SCIBERT 임베딩을 사용하는 모델을 능가한다(또는 유사한 성능)
...
우리는 BASE VOCAB을 사용하여 SCIBERT에 대한 미세 조정 실험을 반복함으로써 도메인 내 과학 어휘의 중요성을 평가한다. 우리는 SCIBERT BASE VOCAB에 대한 최적의 초 매개 변수가 SCIBERT-SCIVOCAB의 초 매개 변수와 종종 일치한다는 것을 발견한다.
...
분리된 어휘(섹션 2)와 BERT-Base(섹션 4)에 대한 개선의 크기를 고려할 때, 우리는 도메인 내 어휘가 도움이 되지만, SCIBERT가 과학적 말뭉치 사전 훈련으로부터 가장 많은 이점을 얻을 수 있다고 의심한다.
...
BERT의 도메인 적응에 대한 최근 연구에는 BIOBERT(Lee et al., 2019)와 CLINICBERT(Alsentzer et al., 2019; Huang et al., 2019)가 포함된다. BIOBERT는 PubMed 요약 및 PMC 전체 텍스트 기사에 대해 교육을 받고 CLINICBERT는 MIMIC-III 데이터베이스의 임상 텍스트에 대해 교육을 받는다(Johnson et al., 2016). 대조적으로, SCIBERT는 시맨틱 스콜라 코퍼스(Ammar et al., 2018)의 114만 개의 생물 의학 및 컴퓨터 과학 논문의 전문에 대해 훈련된다. 또한, SCIBERT는 도메인 내 어휘(SCIVOCAB)를 사용하는 반면, 위에 언급된 다른 모델은 원래 BERT 어휘(BASEVOCAB)를 사용한다.
...
우리는 BERT를 기반으로 한 과학 텍스트에 대한 사전 훈련된 언어 모델인 SCIBERT를 출시했다.     
우리는 과학 영역의 일련의 작업과 데이터 세트에 대해 SCIBERT를 평가했다. SCIBERT는 BERT-Base를 크게 능가하고 생물 의학 작업에 대한 일부 보고된 BIOBERT(Lee et al., 2019) 결과와 비교해도 이러한 작업 중 몇 가지에서 새로운 SOTA 결과를 달성한다.


향후 작업을 위해 BERT-Large와 유사한 SCIBERT 버전을 출시하고 각 도메인의 다양한 비율의 논문을 실험할 예정이다. 이러한 언어 모델은 교육 비용이 많이 들기 때문에 여러 도메인에 걸쳐 유용한 단일 리소스를 구축하는 것을 목표로 한다.
