...
Hierarchical Softmax의 특징은 **softmax를 통해 확률을 구하고자 할 때 분모로 모두 더한다** 방식이다. HHierarchical Softmax는 이를 무시하고 확률을 구하고자 한다. 모델 구조 자체를 full binary tree 구조로 바꾼 후에 단어들은 leaf node에 배치되게 된다.
