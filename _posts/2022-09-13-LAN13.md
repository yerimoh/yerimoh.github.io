...
그렇다면 이 문장에서 **'want'** 라는 중심단어를, window size = 1에 해당하는 주변단어**(I, to)** 들을 사용하여 학습한다고 가정하자.    

그렇다면 아래와 같은 tree에서 학습이 될 것이다.     
편의를 위해 I와 to 중 **I**로 만 를 예시로 들었다.


그럼 위의 트리에서 노드를 끝까지 잘 찾아가 정답인 **I**나 to를 찾아갈 각 edge들의 확률을 높여나가며 학습해나간다.(여기선 I)      
여기서 최종 확률(출력층의 값)은 root부터 leaf까지 가는 길에 있는 확률을 모두 곱하여계산된다.     
즉 I가 나올 확률은 ----이다.
...
즉 위의 식들을 일반화해보면 아래와 같다.
![image](https://user-images.githubusercontent.com/76824611/210940941-be873b70-789e-41b9-b256-850ecda38e00.png)
