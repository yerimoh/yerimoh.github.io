---
title: "[12] Deep learning 2: 추론기반 기법 word2vec (CBOW,skip-gram 모델)"
date:   2020-02-26
...
* 말뭉치 전체의 통계(동시발생 행렬과 PPMI 등)를 이용해 단 1회의 처리 (SVD 등)만에 단어의 분산 표현을 얻음(배치학습)      
...
## CBOW 모델 사용
* ‘모델’을 신경망으로 구축이 목표      
* word2vec에서 제안하는 **CBOW continuous bag-of-words 신경망** 사용 (CBOW 모델과 skip-gram 모델은 word2vec에서 사용되는 신경망)     
...
<details>
<summar🤷‍♀️ 이 그림에서 입력층이 2개인 이유</summary>
<div markdown="1">
...
이때 완전연결계층의 가중치 W in 은 7×3 행렬이며, 이 가중치가 바로 **단어의 분산 표현의 정체.**
...
<details>
<summar👀 **CBOW 모델의 추론 처리** 구현 코드 보기</summary>
<div markdown="1">
...
* 그 **결과로** 가중치 W in 에(정확하게는 W in 과 W out 모두에) 단어의 **출현 패턴을 파악**한 **벡터가 학습**      
...
* **입력 측** 완전연결계층의 가중치(W_in )     
  * 입력 측 가중치 W_in 의 각 **행**: 각 단어의 분산 표        
...
# 인수를 두 개 받음.
# 하나는 단어 ID의 배열(corpus), 다른 하나는 맥락의 윈도우 크기(window _size)
...
$$P(w_t | w_{t−1} ,w_{t+1})$$    
* ‘w_{t-1} 과 w_{t+1} 이 일어난 후 w_t가 일어날 확률’    
