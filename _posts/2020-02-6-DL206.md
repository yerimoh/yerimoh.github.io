...
아래 예시는 3-layer neural network에 여기에 Dropout을 추가했한 것        
...
<details>
<summary>📜 학생 질문: ropout을 사용하게 되면 Train time에서 gradient에는 어떤 일이 일어나는지</summary>
<div markdown="1">
...
* **2) Small Dataset (C classes)**     
   * Imagenet에서 학습된 features를 우리가 가진 작은 데이터셋에 적용하       
   * 이제는 1000개의 ImageNet 카테고리를 분류하는 것이 아니라 10종의 강아지를 분류하는 문제로 바뀐 것이다.(데이터는 엄청 적음)       
   * 이 작은 데이터셋은 C개의 클래스만 가지고있다고 가정해보자.(10개)       
   * 일반적인 절차는, 우선 가장 마지막의 FC Layer는 최종 feature와 class scores간의 연결인데 이를 초기화시킨다.     
* **3) Bigger dataset**      
