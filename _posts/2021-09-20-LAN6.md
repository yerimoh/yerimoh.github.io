...
**(i) 첫번째 대안: model confidence에 따른 Distillation **  
이 접근 방식에서는 모델의 **예측 신뢰도**를 기반으로 샘플을 선택합니다. 이 방법은 [Ruder and Plank(2018a)]()가 제안한 선택 방법과 정신이 유사합니다.    
문장 수준 작업(예: XNLI)의 경우 모델은 각 훈련 예제에 대해 단일 클래스 분포를 생성합니다.    
이 경우 모델의 신뢰도는 p ∗ = maxc∈{1...C} p cθ(x)로 계산됩니다.   
토큰 수준 시퀀스 레이블 지정 작업(예: NER)의 경우 모델의 신뢰도는 다음과 같이 계산됩니다.     
여기서 T는 시퀀스의 길이입니다. 그런 다음 가장 높은 신뢰도 점수를 가진 상위 η% 샘플을 선택하여 증류를 수행합니다. 
The first stage of distillation involves predictions from a single model for which we propose two alternatives: (i) Distillation by model confidence: In this approach, we select samples based on the model’s prediction confidence. This method is similar in spirit to the selection method proposed by Ruder and Plank (2018a). For sentence-level tasks (e.g., XNLI), the model produces a single class distribution for each training example. In this case, the model’s confidence is computed by p ∗ = maxc∈{1...C} p cθ(x). For token-level sequence labeling tasks (e.g., NER), the model’s confidence is computed by: where T is the length of the sequence. The distillation is then done by selecting the top η% samples with the highest confidence scores.
