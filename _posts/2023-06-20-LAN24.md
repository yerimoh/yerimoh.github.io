...
features, we trained a support vector machines (SVM) classifier with radial basis function (RBF) kernel to differentiate between zero vs non-zero band gap materials. Hyperparameter optimization for parameters C (regularization) and γ (inverse of the standard
...
of 90.8 ± 1.0% was obtained for γ = 2.34 and C = 1.83. In fig. S6a we plot a confusion matrix corresponding to a single 5-fold cross-validation applied to a re-shuffled (test)
