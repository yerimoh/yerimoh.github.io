...
* **c.**
원소의 단어 임베딩을 특징으로 하는 간단한 신경망 모델을 사용하여 약 10,000 $$ABC_2D_6$$ elpasolite compounds 의 형성 에너지 예측 대 실제(DFT) 값 (모델의 자세한 내용은 S6 보충 정보 참조).         
그림의 데이터 점은  fivefold cross-validation을 사용함.      
see Supplementary Information section S6 for the details of the model
...
Figure S5: Formation energies of ABC2D6 elpasolites. a. The architecture of the neural network used for predictions. b. Validation and test scores for 4 different feature choices as well as different hidden layer sizes. The performance for word embeddings does not improve much above 800 hidden neurons.
그림 S5: ABC2D6 엘파솔라이트의 형성 에너지.a. 예측에 사용되는 신경망의 아키텍처. b. 서로 다른 숨겨진 레이어 크기뿐만 아니라 4개의 다른 기능 선택에 대한 검증 및 테스트 점수. 단어 임베딩 성능은 800개 이상의 숨겨진 뉴런을 크게 향상시키지 않습니다.



우리는 특징으로 구성 요소의 단어 임베딩(Word2vec 및 Glove 모두 테스트됨)만을 사용하여 55.7 meV/atom의 평균 절대 오차를 가진 엘파졸라이트의 형성 에너지를 예측할 수 있었습니다. 우리는 참조[9]에서 제공하는 약 10,000개의 ABC2D6 자료로 구성된 데이터 세트를 사용합니다. 우리는 ReLU(교정된 선형 단위) 활성화가 있는 단일 완전히 연결된 숨겨진 레이어와 단일 출력 뉴런(그림 S5a)을 참조 [10]과 동일하게 사용합니다. 입력의 경우 A, B, C 및 D 요소의 임베딩을 연결하고 A와 B가 동일하기 때문에 각 재료에 대해 2개의 교육 예제를 만들어 데이터를 보강합니다. 데이터를 교육, 검증 및 테스트 세트로 분할한 후 이 데이터 확대를 수행하여 모든 구별되는 자료가 세트 중 하나에서만 발생하는지 확인하는 것이 중요합니다. 이 확대 방식을 사용하면 최고 성능의 모델 테스트 세트의 평균 절대 오차가 69.2 meV/atom에서 55.7 meV/atom으로 감소합니다. 또한 이전 섹션의 7가지 요소 특성으로 구성된 요소의 원핫 인코딩 및 최소-최대 스케일(0과 1 사이의 모든 특징 값) 벡터와 같은 동일한 신경망 아키텍처로 대체 특징 벡터를 테스트합니다. 숨겨진 레이어의 다양한 크기뿐만 아니라 다양한 기능에 대한 성능이 그림에 요약되어 있습니다. S5b, 단어 임베딩이 최고의 성능을 발휘합니다. 표시된 유효성 검사 점수는 5배 교차 유효성 검사의 평균 절대 오차(MAE)입니다. 교육 전에 분리된 10% 테스트 세트에 대해 테스트 점수가 보고됩니다
We were able to predict formation energies of elpasolites with mean absolute errors as low as 55.7 meV/atom using only word embeddings (both Word2vec and Glove were tested) of their constituent elements as features. We use a dataset with approximately 10,000 ABC2D6 materials available from reference [9]. We use one of the simplest neural network architectures - a single fully connected hidden layer with ReLU (rectified linear unit) activation and a single output neuron (fig. S5a) - the same as reference [10]. For the input we concatenate embeddings of A, B, C and D elements, and also augment the data by creating 2 training examples for each material because A and B are equivalent. It is important to perform this data augmentation after splitting the data into training, validation and test sets to make sure every distinct material occurs only in one of the sets. The mean absolute error on the test set of the best performing model decreases from 69.2 meV/atom to 55.7 meV/atom if we use this augmentation scheme. We also test alternative feature vectors with the same neural network architecture, such as one-hot encoding of elements and min-max scaled (all feature values between 0 and 1) vectors composed of the 7 elemental properties from the previous section. The performances for different features as well as different sizes of the hidden layer are summarized in fig. S5b, with word embeddings clearly performing the best. The displayed validation scores are the mean absolute errors (MAE) for 5-fold cross-validation. The test score is reported for a 10% test set separated before the training
