...
각 materials structure optimization 및 band structure calculation은 Vienna Ab initio Simulation Package(VASP)31,32에서 구현된 프로젝터 증강파(PAW)29 유사 전위와 Perdew-Burke-Ernzerhof(PBE)30 일반화 그레이디언트 근사(GGA)를 사용하여 밀도 함수 이론(DFT)으로 수행되었습니다. 전이금속 산화물 16에는 +U 보정이 적용되었습니다. VoltzTraP 패키지 33을 사용하여 300K와 1300K 사이의 시뮬레이션 온도에서 10-14초의 일정 이완 시간을 사용하고 1016cm-3과 1022cm-3 사이의 캐리어 농도(도핑)에 대해 Seebeck 계수(S)와 전기 전도도(θ)를 계산했습니다. A 48,계산의 770-물질 부분 집합은 이전 작업에서 가져온 것입니다. 16. 나머지 계산은 소프트웨어 atomate34를 사용하여 이 작업에서 수행되었습니다. 모든 계산은 FireWorks35 워크플로 관리 프레임워크 내에서 pymatgen28 Python 라이브러리를 사용했습니다. 후보 물질의 열전 잠재력을 보다 현실적으로 평가하기 위해 S와 δ 텐서의 복잡한 거동을 단일 역률 메트릭으로 응축하는 간단한 전략을 고안했습니다. 각각의 반도체 타입 γ γ {n, p}, 온도 T, 도핑 레벨 c에 대해 S, γ 텐서를 3개의 결정학적 방향에 걸쳐 평균하였고, 평균 역률 PFavg를 계산하였습니다. PFavg는 완벽한 단결정의 역률로부터 다결정 역률을 대략적으로 추정한 것입니다. 그런 다음 T, c 및 γ와 함께 S와 γ의 복잡한 거동을 설명하기 위해 T, c 및 γ에 대한 최대 평균 역률을 최대 차단 온도 Tcut 및 최대 차단 도핑 ccut으로 제한했습니다. 공식적으로, T T cut c, c c ≤ ut T cavg, max, cut cut과 같은 PF m≡ ≤ ax PF(, η Tc, )입니다. 우리는 Tcut = 600K 및 ccut = 1020cm-3을 선택했는데, 이러한 값이 지속적인 이완 시간 근사치의 한계로 인해 더 낙관적인 값보다 실험 데이터 세트와 더 잘 일치하기 때문입니다. 결과 역률 PFavg, max 600K, 1020은 본 연구에서 '계산 역률'과 동일시됩니다. 실험 역률(또는 zT)에 따라 재료의 순위를 매기기 위해, 우리는 Gaultois et al.19의 데이터 세트에 존재하는 모든 실험 조건에서 주어진 화학양론에 대한 최대값을 사용했습니다.


Each materials structure optimization and band structure calculation was performed with density functional theory (DFT) using the projector augmented wave (PAW)29 pseudopotentials and the Perdew– Burke–Ernzerhof (PBE)30 generalized-gradient approximation (GGA), implemented in the Vienna Ab initio Simulation Package (VASP)31,32. A +U correction was applied to transition metal oxides16. Seebeck coefficient (S) and electrical conductivity (σ) were calculated using the BoltzTraP package33 using a constant relaxation time of 10−14 s at simulated temperatures between 300 K and 1,300 K and for carrier concentrations (doping) between 1016 cm−3 and 1022 cm−3 . A 48,770-material subset of the calculations was taken from a previous work16; the remaining calculations were performed in this work using the software atomate34. All calculations used the pymatgen28 Python library within the FireWorks35 workflow management framework. To more realistically evaluate the thermoelectric potential of a candidate material, we devised a simple strategy to condense the complex behaviour of the S and σ tensors into a single power factor metric. For each semiconductor type η ∈ {n, p}, temperature T, and doping level c, the S and σ tensors were averaged over the three crystallographic directions, and the average power factor, PFavg, was computed. PFavg is a crude estimation of the polycystalline power factor from the power factor of a perfect single crystal. To account for the complex behaviour of S and σ with T, c, and η, we then took the maximum average power factor over T, c, and η constrained to a maximum cutoff temperature Tcut and maximum cutoff doping ccut. Formally, this is PF m ≡ ≤ ax PF( , η T c, ) such that T Tcut c ,c c ≤ ut T c avg, max , cut cut . We chose Tcut = 600 K and ccut = 1020 cm−3 because these values resulted in better correspondence with the experimental dataset than more optimistic values, owing to the limitations of the constant relaxation time approximation. The resulting power factor, PFavg, max 600K, 10 20 , is equated with ‘computed power factor’ in this study. To rank materials according to experimental power factors (or zT), we used the maximum value for a given stoichiometry across all experimental conditions present in the dataset from Gaultois et al.19.
