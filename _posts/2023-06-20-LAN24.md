...
추상화의 주요 목적은 교육 중 임베딩에서 노이즈를 증가시킬 수 있는 불필요한 단어를 피하면서 정보를 간결하고 직접적인 방식으로 전달하는 것입니다. 말뭉치 선택의 중요성은 확장 데이터 표 4에서 입증되는데, 여기서 우리는 무기 재료 과학과 관련이 없는 추상물을 폐기하는 것이 성능을 향상시키고 모든 위키백과 기사 세트에 대해 훈련된 모델(우리 말뭉치보다 약 10배 많은 텍스트)이 재료 과학 유추에서 상당히 더 나쁜 성능을 발휘한다는 것을 보여줍니다. 기존의 기계 학습 모토와 달리 문제에 더 많은 데이터를 던지는 것이 항상 해결책은 아닙니다. 대신, 말뭉치의 품질과 도메인 특정성이 도메인 특정 작업에 대한 임베딩의 유용성을 결정합니다
