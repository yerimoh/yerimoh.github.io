...
다음으로, 동일한 모델을 실험적으로 측정된 power factorsd를 [$$zTs^{19}$$](https://pubs.acs.org/doi/10.1021/cm400893e)와 직접 비교했다.        


우리의 접근 방식은 이러한 양에 대한 수치 추정을 제공하지 않기 때문에, 우리는 텍스트 말뭉치와 실험 세트에 모두 나타나는 83가지 재료에 대해 스피어맨 순위 상관 관계 20을 통해 후보자의 상대적 순위를 비교했습니다. 최대 역률 및 최대 zT에 대한 임베딩 기반 순위와 실험 결과의 59% 및 52% 순위 상관 관계를 얻었습니다. 예상치 못하게 우리 모델은 이전 단락에서 사용된 역률의 DFT 데이터 세트를 능가했는데, 이는 실험적인 최대 역률과 31%의 순위 상관관계만 나타냅니다.
...
마지막으로, 우리는 우리의 모델이 과거에 다양한 지점에서 훈련을 받았다면 나중에 문헌에 보고된 열전 물질을 올바르게 예측했을 것인지를 테스트했습니다. 구체적으로, 우리는 2001년과 2018년 사이에 차단 연도 이전에 출판된 추상적인 것으로만 구성된 18개의 다른 '역사적' 텍스트 말뭉치를 생성했습니다. 우리는 각 과거 데이터 세트에 대해 별도의 단어 임베딩을 훈련했고, 이러한 임베딩을 사용하여 미래(테스트) 연도에 보고될 가능성이 있는 상위 50개 열전기를 예측했습니다. 예측 날짜가 지난 매년, 우리는 열전 키워드와 함께 문헌에 보고된 예측 열전 성분의 누적 백분율을 표로 작성했습니다. 그림 3a는 이러한 각 '과거' 데이터 세트의 결과를 얇은 회색 선으로 보여줍니다. 예를 들어, '2015'라는 레이블이 붙은 옅은 회색 선은 2015년 1월 1일 이전에 발표된 과학적 추상물에 대해서만 훈련된 모델을 사용하여 상위 50개 예측 중 1년, 2년, 3년 또는 4년 후(즉 2015-2018년) 열전 키워드와 함께 문헌에 보고된 비율을 나타냅니다. 전반적으로. 우리의 결과는 상위 50개 단어 임베딩 기반 예측(빨간색 선)의 재료가 그 당시 우리의 말뭉치에서 무작위로 선택된 연구되지 않은 재료(파란색)와 비교하여 향후 5년 내에 열전기로 연구될 가능성이 평균 8배 더 높았고 또한 무작위 재료보다 3배 더 높다는 것을 나타냅니다 0이 아닌 DFT 밴드갭(녹색)을 사용합니다. 그림 3a에서 나중에 더 가파른 기울기로 표시된 것처럼, 더 최근의 데이터를 통합하는 더 큰 말뭉치의 사용은 성공적인 예측의 속도를 향상시켰습니다.
