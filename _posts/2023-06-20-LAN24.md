...
* **fig detail**      
   ...
   * material는 첫 번째(Li2CuSb), 세 번째(CsAgGa2Se4) 및 네 번째(Cu7Te5) 예측입니다. 문맥어는 재료와 '열전성' 단어 사이의 가장자리 가중치의 합에 따른 최상위 문맥어입니다. 경로가 넓을수록 예측에 더 큰 기여를 할 것으로 예상됩니다. 컨텍스트 워드를 검사하면 알고리즘이 결정 구조 연관성, 동일한 응용 프로그램에 대한 다른 재료와의 공동 언급, 서로 다른 응용 프로그램 간의 연관성 및 재료의 알려진 특성을 설명하는 주요 구문을 기반으로 예측을 수행하고 있음을 알 수 있습니다.
