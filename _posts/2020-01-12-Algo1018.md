...
이렇게 3가지 로직이다. 이외에도 앞뒤로 처리를 쉽게 하기 위한 문자열을 문자 단위 리스트로 분리하여 중첩 리스트 Nested List 표현, 빈 공간에 대한 개수 카운팅을 추가하면 모든 구현을 완료할 수 있다.
...
board = [list(x) for x in board]
matched = True while matched:
# 1) 일치 여부 판별 matched = [] for i in range(m - 1):
for j in range(n - 1):
if board[i][j] == \ board[i][j + 1] == \ board[i + 1][j + 1] == \ board[i + 1][j] != '#':
matched.append([i, j])
# 2) 일치한 위치 삭제 for i, j in matched:
board[i][j] = board[i][j + 1] = board[i + 1][j + 1] = board[i + 1][j] = '#'
# 3) 빈공간 블럭 처리 for _ in range(m):
for i in range(m - 1):
for j in range(n):
if board[i + 1][j] == '#':
board[i + 1][j], board[i][j] = board[i][j], '#'
return sum(x.count('#') for x in board)
이 문제는 복잡한 비즈니스 로직으로 인해 당시 제출 코드의 길이가 가장 길었던 문제로, 자바의 경우 무려 80라인에 달한다. 파이썬은 평균 48라인이었는데, 지금 이 풀이는 비즈니스 로직 설명을 위한 주석을 포함한다 해도 29라인에 불과하니 상당히 잘 풀이했다고 자화자찬할 수 있을 것 같다. 적절히 리스트 컴프리헨션을 활용해 전체 길이를 줄였을 뿐만 아니라 추가 변수를 사용하지 않아 공간 복잡도 또한 최소화했다. 이 문제의 정답률은 48.01%였다
