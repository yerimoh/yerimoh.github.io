...
트랜스포머의 주요 구성 요소는 자가 주의입니다(Cheng et al., 2016). 자기주의는 각 요소를 나머지 시퀀스의 가중 평균으로 대체하여 시퀀스를 처리하는 주의의 변형이다(Graves, 2013; Bahdanau et al., 2015). 원래 트랜스포머는 인코더-디코더 아키텍처로 구성되었으며 시퀀스 대 시퀀스 작업을 위해 고안되었다(Sutskever et al., 2014; Kalchbrenner et al., 2014). 또한 최근에는 언어 모델링(Radford et al., 2018; Al-Rfou et al., 2019) 또는 분류 및 범위 예측 작업(Devlin et al., 2018; Yang et al., 2019)에 적합한 아키텍처를 생성하는 데 사용되는 다양한 형태의 자체 주의를 통해 단일 Transformer 레이어 스택으로 구성된 모델을 사용하는 것이 일반화되었다. 우리는 섹션 3.2에서 이러한 아키텍처 변형을 경험적으로 탐구한다
...
전반적으로, 우리의 인코더-디코더 트랜스포머 구현은 원래 제안된 형태를 밀접하게 따른다(Vaswani et al., 2017). 먼저 토큰의 입력 시퀀스를 임베딩 시퀀스에 매핑한 다음 인코더로 전달한다. 인코더는 블록의 스택으로 구성되며, 각각은 두 개의 하위 구성 요소로 구성된다: 자기 주의 계층과 작은 피드 포워드 네트워크. 각 하위 구성요소의 입력에는 계층 정규화(Ba et al., 2016)가 적용된다. 활성화가 재조정되고 추가 편향이 적용되지 않는 단순화된 버전의 계층 정규화를 사용한다. 레이어 정규화 후, 잔여 스킵 연결(He et al., 2016)은 각 하위 구성요소의 입력을 출력에 추가한다. 드롭아웃(Srivastava et al., 2014)은 피드포워드 네트워크 내에서 스킵 연결, 주의 가중치 및 전체 스택의 입력 및 출력에 적용된다. 디코더는 인코더의 출력에 참여하는 각 자기 주의 계층 다음에 표준 주의 메커니즘을 포함한다는 점을 제외하고는 인코더와 구조가 유사하다. 디코더의 자기 주의 메커니즘은 또한 모델이 과거 출력에만 주의를 기울일 수 있도록 하는 자기 회귀 또는 인과적 자기 주의의 형태를 사용한다. 최종 디코더 블록의 출력은 소프트맥스 출력을 가진 고밀도 레이어로 공급되며, 그 가중치는 입력 임베딩 매트릭스와 공유된다. 트랜스포머의 모든 주의 메커니즘은 추가로 처리되기 전에 출력이 연결되는 독립적인 "헤드"로 분할됩니다.
