...
NLP에 대한 이전 학습에 대한 이전 연구의 대부분은 비지도 학습을 위해 레이블이 지정되지 않은 대규모 데이터 세트를 사용한다. 본 논문에서는 레이블이 지정되지 않은 데이터의 품질, 특성 및 크기의 효과를 측정하는 데 관심이 있다. 우리의 요구를 충족하는 데이터 세트를 생성하기 위해 공통 크롤을 웹에서 스크랩된 텍스트 소스로 활용한다. 커먼 크롤은 이전에 NLP의 텍스트 데이터 소스로 사용되었으며, 예를 들어 n-gram 언어 모델(Buck et al., 2014), 상식 추론을 위한 훈련 데이터(Trinh and Le, 2018), 기계 번역을 위한 병렬 텍스트 마이닝을 위한 훈련 데이터(Smith et al., 2013), 사전 훈련 데이터 세트(Grave et al., 2018; Zellers et al.)로 사용되었다. 2019; Rui et al., 2019c), 그리고 심지어는 최적화 프로그램을 테스트하기 위한 거대한 텍스트 코퍼스로서도 그렇다(Anil et al., 2019).

커먼 크롤(Common Crowle)은 일반적으로 사용 가능한 웹 아카이브로, 스크랩된 HTML 파일에서 마크업 및 기타 비텍스트 콘텐츠를 제거하여 "웹 추출 텍스트"를 제공한다. 이 프로세스는 매달 약 20TB의 스크랩된 텍스트 데이터를 생성합니다. 불행하게도, 결과 텍스트의 대부분은 자연어가 아니다. 대신 메뉴, 오류 메시지 또는 중복 텍스트와 같은 횡설수설하거나 보일러 플레이트 텍스트로 구성됩니다. 또한, 스크랩된 텍스트의 상당 부분은 우리가 고려하는 작업(공격적인 언어, 자리 표시자 텍스트, 소스 코드 등)에 도움이 될 것 같지 않은 내용을 포함한다. 이러한 문제를 해결하기 위해 Common Crow의 웹 추출 텍스트를 정리하기 위해 다음 휴리스틱을 사용했습니다:

• • 마침표, 느낌표, 물음표 또는 끝 따옴표로 끝나는 행만 유지했습니다.

• • 우리는 5개 미만의 문장이 있는 페이지는 폐기하고 최소 3개의 단어가 포함된 행만 유지했습니다.

• • "더티, 장난, 외설 또는 기타 나쁜 단어 목록"에 있는 모든 단어가 포함된 페이지를 제거했습니다.6

• • 대부분의 스크랩 페이지에는 Javascript를 활성화해야 한다는 경고가 포함되어 있어 Javascript라는 단어가 있는 줄은 모두 제거했습니다.

• • 일부 페이지에는 플레이스홀더 "lorem ipsum" 텍스트가 포함되어 있으며, "lorem ipsum"이라는 문구가 나타나는 페이지는 모두 제거했습니다.

• • 일부 페이지에는 실수로 코드가 포함되어 있습니다. 곱슬곱슬한 괄호 "{"는 웹에서 널리 사용되는 자바스크립트와 같은 많은 프로그래밍 언어에서는 나타나지만 자연스러운 텍스트에서는 나타나지 않기 때문에 곱슬곱슬한 괄호가 포함된 모든 페이지를 제거했다.

• • 데이터 세트의 중복을 제거하기 위해 데이터 세트에서 두 번 이상 발생하는 세 문장 범위 중 하나를 제외하고 모두 삭제했습니다.
