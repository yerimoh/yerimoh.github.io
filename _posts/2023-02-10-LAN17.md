...
자가 주의는 순서에 구애받지 않으므로(즉, 세트에 대한 작업), 변압기에 명시적인 위치 신호를 제공하는 것이 일반적입니다. 원래 트랜스포머는 사인파 위치 신호 또는 학습된 위치 임베딩을 사용했지만, 최근에는 상대 위치 임베딩을 사용하는 것이 더 일반적입니다(Shaw et al., 2018; Huang et al., 2018a). 각 위치에 대해 고정 임베딩을 사용하는 대신, 상대적 위치 임베딩은 자체 주의 메커니즘에서 비교되는 "키"와 "쿼리" 사이의 오프셋에 따라 다른 학습된 임베딩을 생성합니다. 우리는 각 "임베딩"이 주의 가중치 계산에 사용되는 해당 로짓에 단순히 추가되는 스칼라인 단순한 형태의 위치 임베딩을 사용합니다. 또한 효율성을 위해 모델의 모든 레이어에 걸쳐 위치 임베딩 매개 변수를 공유하지만, 주어진 레이어 내에서 각 주의 헤드는 서로 다른 학습된 위치 임베딩을 사용합니다. 일반적으로, 각각 가능한 키 쿼리 오프셋 범위에 해당하는 고정된 수의 임베딩이 학습됩니다. 이 연구에서, 우리는 로그적으로 최대 128의 오프셋까지 크기가 증가하는 범위를 가진 모든 모델에 32개의 임베딩을 사용하여 모든 상대 위치를 동일한 임베딩에 할당합니다. 특정 계층은 128개 토큰을 초과하는 상대적 위치에 민감하지 않지만, 후속 계층은 이전 계층의 로컬 정보를 결합하여 더 큰 오프셋에 민감하게 반응할 수 있습니다. 요약하자면, 우리의 모델은 레이어 노름 바이어스를 제거하고 레이어 정규화를 잔류 경로 외부에 배치하고 다른 위치 임베딩 방식을 사용하는 것을 제외하고 Vaswani et al.(2017)이 제안한 원래 트랜스포머와 대략 동등합니다. 이러한 아키텍처 변화는 전이 학습에 대한 경험적 조사에서 고려하는 실험 요소와 직교하기 때문에, 우리는 향후 작업을 위해 영향의 절제를 남겨둡니다.
