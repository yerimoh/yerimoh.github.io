...
* <span style="background-color:#F5F5F5">**distinguishing factor for different architecture: encoder-decoder Transformer**</span>     
  ...
  * **encoder:** **“fully-visible” attention mask**를 사용 (Figure 3)      
        ...
        * 이와 같이, 트랜스포머 디코더(인코더 없음)는 언어 모델(LM), 즉 다음 단계 예측만을 위해 훈련된 모델로 사용될 수 있다.(igure 4 두 번째 모델 구조)         


* <span style="background-color:#F5F5F5">**masking pattern 변경: Causal with prefix masking**</span>     
  * **기존 causal mask의 문제:**     
   ...
   * **Causal with prefix masking로 해결:** (Figure 4)     
   prefix 부분만 fully-visible masking을 사용합니다. 이 마스킹 패턴과 결과적으로 "접두사 LM"(우리가 고려하는 세 번째 모델 구조)의 도식은 각각 그림 3과 4의 맨 오른쪽 패널에 나와 있습니다. 위에서 언급한 영어에서 독일어로의 번역 예제에서 완전히 가시적인 마스킹은 "영어를 독일어로 번역: 좋습니다. target:" 그리고 인과적 마스킹은 목표 "다시스트 내장"을 예측하기 위해 훈련 중에 사용될 것입니다 텍스트 대 텍스트 프레임워크에서 접두사 LM을 사용하는 것은 원래 Liu et al.(2018)에 의해 제안되었습니다. 더 최근에, Dong et al. (2019)은 이 아키텍처가 광범위한 텍스트 대 텍스트 작업에 효과적이라는 것을 보여주었습니다. 이 아키텍처는 인코더-디코더 모델과 유사하며, 인코더-디코더 간에 매개 변수를 공유하고, 입력 및 대상 시퀀스 전반에 걸쳐 인코더-디코더 주의가 완전히 대체되었습니다.

우리는 텍스트 대 텍스트 프레임워크를 따를 때 접두사 LM 아키텍처가 분류 작업을 위한 BERT(Devlin et al., 2018)와 매우 유사하다는 점에 주목합니다. 그 이유를 보려면 MNLI 벤치마크의 예를 생각해 보십시오. 여기서 전제는 "나는 비둘기를 싫어합니다."이고, 가설은 "비둘기에 대한 내 감정은 적대감으로 가득 차 있습니다."이고 올바른 레이블은 "유인"입니다. 이 예를 언어 모델에 입력하기 위해, 우리는 그것을 시퀀스 "mnli 전제: 저는 비둘기가 싫어요. 가설: 비둘기에 대한 나의 감정은 적대감으로 가득 차 있습니다. 대상: 수반." 이 경우, 완전히 보이는 접두사는 단어 "target:"까지 전체 입력 시퀀스에 해당하며, 이는 BERT에서 사용되는 "분류" 토큰과 유사한 것으로 볼 수 있습니다. 따라서, 우리의 모델은 전체 입력에 대해 완전한 가시성을 가진 다음 "유인"이라는 단어를 출력하여 분류를 만드는 작업을 수행할 것입니다. 모델이 작업 접두사(이 경우 "mnli")가 지정된 유효한 클래스 레이블 중 하나를 출력하는 방법을 배우는 것은 쉽습니다.
