...
우리는 또한 단어 유사성 작업에 대한 하위 단어 정보를 통합하는 단어 벡터에 대한 이전 작업과 우리의 접근 방식을 비교한다. 사용된 방법은 Luong et al.(2013)의 재귀 신경망, Quu et al.(2014)의 형태소궁 및 Soricut과 Och의 형태소 변환(2015)이다. 결과를 비교 가능하게 만들기 위해, 우리는 우리가 비교하고 있는 방법과 동일한 데이터 세트, 즉 Shaoul과 Westbury가 발표한 영어 위키피디아 데이터(2010)와 독일어, 스페인어 및 프랑스어를 위한 2013 WMT 공유 작업의 뉴스 크롤 데이터에 대해 모델을 훈련시켰다. 우리는 또한 유럽 연합 뉴스 해설 코퍼스에서 훈련된 Botha와 Blunsom(2014)이 도입한 로그 이진 언어 모델과 우리의 접근 방식을 비교한다. 다시, 우리는 결과를 비교할 수 있도록 동일한 데이터에 대해 모델을 훈련시켰다. 우리의 모델을 사용하여, 우리는 문자 n-그램의 표현을 합산하여 어휘 부족 단어의 표현을 얻는다. 우리는 결과를 표 3에 보고한다. 우리는 우리의 단순한 접근 방식이 형태론적 세그먼트에서 얻은 하위 단어 정보를 기반으로 하는 기술에 비해 잘 수행된다는 것을 관찰한다. 우리는 또한 우리의 접근 방식이 접두사 및 접미사 분석을 기반으로 하는 Soricut and Och(2015) 방법을 능가한다는 것을 관찰한다. 독일어가 크게 향상된 것은 그들의 접근 방식이 우리의 접근 방식과 반대로 명사 합성을 모델링하지 않기 때문이다.
