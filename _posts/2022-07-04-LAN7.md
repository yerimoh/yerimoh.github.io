...
모델에 의해 D가 예측되어야 하는 D와 마찬가지로 A가 B에 대한 형태의 단어 유추 질문에 대한 접근 방식을 평가한다. 우리는 영어를 위해 Mikolov et al.(2013a), 체코를 위해 Svoboda와 Brychin(2016), 독일을 위해 Köper et al.(2015), 이탈리아를 위해 Berardi et al.(2015)에 의해 도입된 데이터 세트를 사용한다. 일부 질문에는 훈련 말뭉치에 나타나지 않는 단어가 포함되어 있으므로 이러한 질문을 평가에서 제외했다.
