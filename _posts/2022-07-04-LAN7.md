...
독일어의 경우 GUR65, GUR350 및 ZG222의 세 가지 데이터 세트에서 서로 다른 모델을 비교한다(Gurevych, 2005; Zesch and Gurevych, 2006). 영어의 경우, 우리는 Finkelstein 등이 소개한 WS353 데이터 세트를 사용한다. (2001) 및 Luong 등(2013)이 도입한 희귀 단어 데이터 세트(RW). 우리는 번역된 데이터 세트 RG65에서 프랑스어 단어 벡터를 평가한다(Joubarne and Inkpen, 2011). 스페인어, 아랍어 및 루마니아어 단어 벡터는 (Hassan and Mihalcea, 2009)에 설명된 데이터 세트를 사용하여 평가된다. 러시아어 단어 벡터는 판첸코 외 연구진(2016)이 도입한 HJ 데이터 세트를 사용하여 평가된다.
