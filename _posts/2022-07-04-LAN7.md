...
우리는 단어 간의 문자 수준 유사성을 활용하기 때문에 자주 사용하지 않는 단어를 더 잘 모델링할 수 있다. 따라서, 우리는 또한 우리가 사용하는 훈련 데이터의 크기에 더 강건해야 한다. 이를 평가하기 위해, 우리는 훈련 데이터 크기의 함수로서 유사성 작업에 대한 단어 벡터의 성능을 평가할 것을 제안한다. 이를 위해, 우리는 위키피디아의 크기가 증가하는 부분에서 모델과 cbow 기준선을 훈련시킨다. 우리는 위에서 설명한 위키피디아 말뭉치를 사용하고 데이터의 첫 번째 1, 2, 5, 10, 20, 50%를 분리한다. 우리는 데이터 세트를 개편하지 않기 때문에, 그것들은 모두 서로의 하위 집합이다. 우리는 결과를 그림 1에 보고한다.
