...
먼저 단어 유사성/관련성 작업에서 표현의 품질을 평가한다. 우리는 인간의 판단과 벡터 표현 사이의 코사인 유사성 사이의 스피어맨의 순위 상관 계수(Spearman, 1904)를 계산함으로써 그렇게 한다. 독일어의 경우 GUR65, GUR350 및 ZG222의 세 가지 데이터 세트에서 서로 다른 모델을 비교한다(Gurevych, 2005; Zesch and Gurevych, 2006). 영어의 경우, 우리는 Finkelstein 등이 소개한 WS353 데이터 세트를 사용한다. (2001) 및 Luong 등(2013)이 도입한 희귀 단어 데이터 세트(RW). 우리는 번역된 데이터 세트 RG65에서 프랑스어 단어 벡터를 평가한다(Joubarne and Inkpen, 2011). 스페인어, 아랍어 및 루마니아어 단어 벡터는 (Hassan and Mihalcea, 2009)에 설명된 데이터 세트를 사용하여 평가된다. 러시아어 단어 벡터는 판첸코 외 연구진(2016)이 도입한 HJ 데이터 세트를 사용하여 평가된다.
...
우리는 모든 데이터 세트에 대한 방법과 기준선에 대한 결과를 표 1에 보고한다. 이러한 데이터 세트의 일부 단어는 교육 데이터에 나타나지 않으므로 cbow 및 skipgram 기준선을 사용하여 이러한 단어에 대한 단어 표현을 얻을 수 없다. 비교 가능한 결과를 제공하기 위해 기본적으로 이러한 단어에 null 벡터를 사용할 것을 제안한다. 우리 모델은 하위 단어 정보를 활용하기 때문에 어휘 부족 단어에 대한 유효한 표현도 계산할 수 있다. 우리는 n-그램 벡터의 합을 취함으로써 그렇게 한다. OOV 단어가 null 벡터를 사용하여 표현되는 경우, 우리는 우리의 방법을 sisg- 및 sisg라고 달리 부른다(하위 단어 정보 스킵 그램). 첫째, 표 1을 보면 하위 단어 정보를 사용하는 제안된 모델(sisg)이 영어 WS353 데이터 세트를 제외한 모든 데이터 세트의 기준선을 능가한다는 것을 알 수 있다. 또한, 어휘 부족 단어(sisg)에 대한 벡터 계산은 적어도 항상 그렇지 않은 것(sisg-)만큼 좋다. 이것은 문자 n그램의 형태로 하위 단어 정보를 사용하는 것의 이점을 증명한다.
...
둘째, 영어, 프랑스어 또는 스페인어보다 아랍어, 독일어 및 러시아어에서 문자 n-그램을 사용하는 효과가 더 중요하다는 것을 관찰한다. 독일어와 러시아어는 문법적으로 차이가 있는데, 독일어의 경우 4개, 러시아어의 경우 6개이다. 예를 들어, "탁구"라는 명목상의 문구는 "티슈테니스"라는 단어로 쓰여진다. "Tischtennis"와 "Tennis"의 문자 수준 유사성을 활용하여, 우리의 모델은 두 단어를 완전히 다른 단어로 표현하지 않는다.
...
마지막으로, 우리는 영어 희귀 단어 데이터 세트(RW)에서 우리의 접근 방식이 영어 WS353 데이터 세트에서는 그렇지 않은 반면 기준선을 능가한다는 것을 관찰한다. 이는 영어 WS353 데이터 세트의 단어가 하위 단어 정보를 활용하지 않고도 좋은 벡터를 얻을 수 있는 일반적인 단어이기 때문이다. 빈도가 낮은 단어를 평가할 때, 단어 간의 문자 수준에서 유사성을 사용하면 좋은 단어 벡터를 학습하는 데 도움이 될 수 있음을 알 수 있다.
