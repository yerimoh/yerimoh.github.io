...
- [Transformer 모델의 시각화](#transformer-모델의-시각화)
  * [모델의 전체 개요](#모델의-전체-개요)
    ...
    + [encoder 확대](#encoder-확대)
  * [decoder확대](#decoder-확대)
- [백터들을 기준으로 그림 그려보기](#백터들을-기준으로-그림-그려보기)

- [encoder의 각 sub-layer](#encoder의-각-sub-layer)
  * [sub-layer1 > Self-Attention](#sub-layer1---self-attention)
  * [The Beast With Many Heads /  "multi-headed” attention](#the-beast-with-many-heads-/--multi-headed--attention)
...
- [Decoder](#decoder)
  * [Encoder-Decoder Attention layer](#encoder-decoder-attention-layer)
  * [Linear Layer과 Softmax Layer](#linear-layer--softmax-layer)
...
### self-attention 원리 1 (벡터 계산)      
여러 가지 벡터들을 통해서 어떻게 self-attention 을 계산할 수 있을까?     
* 행렬을 이용해서 이것이 실제로 어떻게 구현돼 있는지 확인   '

실제로는 벡터단위로 계산을 안하고 행렬 단위로 계싼을 하지만 이해를 위해 백터계산을 설면 항 후 행렬 계산을 보여주겠다.   
...
### self-attention 원리 2 (행렬 계산) 
**실제 구현**에서는 빠른 속도를 위해 이 모든 과정들이 ~~벡터~~가 아닌 **행렬**의 형태로 진행    
...
## self-attention_multi-headed attention
...
![image](https://user-images.githubusercontent.com/76824611/117900128-8d30ee00-b303-11eb-938d-dd20614ca183.png)


**[총 정리]**  
![image](https://user-images.githubusercontent.com/76824611/117901910-4c3ad880-b307-11eb-9cac-d1f28f8e3235.png)
...
**[우리가 이때까지 설명해온 Transformer 모델의 problem]**   
이 모델이 입력 문장에서 **단어들의 순서**에 대해서 고려하고 있지 않음
...
![image](https://user-images.githubusercontent.com/76824611/117903955-75f5fe80-b30b-11eb-8031-e17fab51cd58.png)

---

## The Residuals
* **error(오차)**: **모집단**에서 회귀식을 얻었다면, 그 회귀식을 통해 얻은 예측값과 실제 관측값의 차이     
* **Residual(진치)**: **표본집단**에서 회귀식을 얻었다면, 그 회귀식을 통해 얻은 예측값과 실제 관측값의 차이       
...
![image](https://user-images.githubusercontent.com/76824611/117926820-53c4a680-b334-11eb-868f-ce1f86df9d92.png)

----

# 전체 코드
![image](https://user-images.githubusercontent.com/76824611/117930352-d9e2ec00-b338-11eb-9558-c41b44c2dd4b.png)
...
![image](https://user-images.githubusercontent.com/76824611/117931741-86719d80-b33a-11eb-8833-d6d489c236b1.png)
...
*  각 스텝마다의 출력된 단어는,
 ...
 ![image](https://user-images.githubusercontent.com/76824611/117931770-8ec9d880-b33a-11eb-9b86-854d7badae7f.png)


### decoder 안의 self-attention layer
encoder와는 조금 다르게 작동      
*  output sequence 내에서 **현재 위치의 이전 위치들에 대해서만** attend 할 수 있음
*  이건, self-attention 계산 과정에서 softmax를 취하기 전에, **현재 스텝 이후**의 위치들에 대해서 **masking** (즉 그에 대해서 $$-inf$$로 치환하는 것)을 해줌으로써 가능해짐    
...
**[한가지 차이점]**    
Query 행렬들을 그 밑의 layer에서 가져오고 Key 와 Value 행렬들을 encoder의 출력에서 가져온다는 점       
...
![image](https://user-images.githubusercontent.com/76824611/117934274-77d8b580-b33d-11eb-941e-64ea370d5856.png)
