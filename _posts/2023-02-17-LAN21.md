...
<span style="background-color:#F5F5F5">**[Materials caption graph]**</span>      
* 앞에서 언급한 쿼리 외에도 그래프 표현은 그림 캡션에 퍼져 있는 정보에 대한 심층적인 통찰력을 제공할 수 있다. 예를 들어, "특정 재료에 대해 어떤 합성 및 특성화 방법이 일반적으로 사용되는가?", "특정 특성을 측정하는 방법은 무엇인가?"와 같은 질문은 지식 그래프를 사용하여 쉽게 대답할 수 있다. 여기서는 재료 캡션 그래프(MCG)를 사용하여 그림 캡션의 정보를 어떻게 표현할 수 있는지 보여준다. 이 정도로, 우리는 먼저 유리 관련 출판물에서 10,000개의 그림 캡션을 무작위로 선택한다. 또한 Matscholar NER 데이터 세트에서 미세 조정된 MatSciBERT를 사용하여 그림 캡션에서 엔티티와 유형을 추출한다. 각 캡션에 대해 해당 캡션에 있는 모든 엔티티를 연결하여 완전히 연결된 그래프를 만듭니다. 그런 다음 이 그래프들은 함께 결합되어 큰 MCG를 형성한다. 우리는 아래 MCG에서 얻은 몇 가지 통찰력을 보여준다.

그림 5는 MCG에서 추출한 두 개의 그래프 하위 집합을 보여줍니다. 그림 5a에서, 우리는 2홉 이웃인 두 개의 실체, 즉 Tg와 어닐을 식별했다. 이러한 엔티티는 가장자리를 공유하지 않습니다. 즉, 주어진 캡션에서 이 두 엔티티가 동시에 발견되지 않습니다. 그런 다음 두 노드의 모든 원홉 이웃의 교차점을 식별하고 그림 5a와 같이 그래프를 표시했다. 가장자리의 두께는 발생 횟수 측면에서 연결 강도를 나타냅니다. 우리는 Tg와 어닐에 대한 4개의 일반적인 1홉 이웃, 즉 XRD, 도핑, 유리 및 비정질이 있음을 관찰한다. 이는 그래프를 생성하는 데 사용되는 캡션에서 이 두 엔티티가 직접 연결되지 않더라도 Tg 및 어닐과 함께 캡션에서 이 네 엔티티가 발생한다는 것을 의미합니다. 도 5a는 Tg가 유리, 비정질 및 도핑된 물질과 관련이 있으며, 이들 물질은 어닐링에 의해 합성될 수 있음을 시사한다. 마찬가지로 어닐링에 의해 얻어진 구조는 XRD로 특징지을 수 있다. 이러한 결과로부터, 우리는 또한 유리 과학의 전통적인 지식과 일치하는 어닐링의 영향을 받는다는 것을 추론할 수 있다.

마찬가지로, 도 5b는 노드 XRD에 연결된 모든 엔티티를 보여준다. 이 정도로, 우리는 XRD를 갖는 모든 캡션을 CMT로 선택하고, 그 캡션의 모든 엔티티를 얻은 후, 그림 5b와 같이 무작위로 20쌍을 샘플링한 다음 플롯한다. 한 쌍(XRD, XRD)과 두 개의 유사한 쌍(XRD, 유리)이기 때문에 가장자리 수는 18개이고 노드 수는 19개입니다. 노드 색은 엔티티 유형을 나타내고, 가장자리 너비는 "XRD"가 있는 캡션에서 추출된 엔티티의 전체 데이터베이스에서 쌍의 빈도를 나타냅니다. 그래프를 사용하여 우리는 다음과 같은 정보를 얻을 수 있다

1. XRD는 유리, 도핑된 물질, 나노 섬유 및 필름과 같은 다양한 물질 설명자의 특성화 방법으로 사용됩니다.
...
5. ZnO, 안경, CsPBr3, 이트리아 부분 안정화 지르코니아(YPSZ)와 같은 다양한 재료의 경우, XRD는 더 두꺼운 가장자리 폭에서 명백한 주요 CMT입니다.

이 정보는 재료 문헌에서 광범위한 재료 및 응용 분야를 다루고 있습니다. 재료 문헌에 대한 통찰력을 얻기 위해 MCG를 사용하여 서로 다른 엔티티 및 엔티티 유형에 대해 유사한 그래프를 생성할 수 있다.
