...
implementation of PyTorch-CRF library62.
...
BiLSTM is finally fed to the CRF layer to make predictions.
...
* LM-선형: 워드피스의 출력 임베딩은 소프트맥스 활성화와 함께 선형 레이어를 통과한다. 우리는 트랜스포머 라이브러리 57의 BERT 토큰 분류기 구현을 사용한다.
...
* LM-CRF: 모델이 언급된 동일한 엔티티에 속하는 토큰에 레이블을 지정하고 다른 엔티티 유형 간의 전환 점수를 학습할 수 있도록 LM-선형 아키텍처의 최종 softmax 활성화를 CRF 레이어 61로 대체한다. 우리는 PyTorch-CRF 라이브러리62의 CRF 구현을 사용한다.

* LM-BiLSTM-CRF: 양방향 장단기 메모리63은 LM과 CRF 층 사이에 추가된다. 모든 워드피스의 BERT 임베딩은 적층된 BiLSTM을 통해 전달된다. BiLSTM의 출력은 예측을 하기 위해 마지막으로 CRF 계층에 공급된다.
...
proposed by Maini et al. (2020)50. In this approach too, the pair of entities
...
In Paper Abstract Classification task, we use the output embedding of
the CLS token to encode the entire text/abstract. We pass this embedding
through a simple classifier to make predictions. We use the BERT Sentence
Classifier implementation of the transformers library57. For the baseline, we
use a similar approach as relation classification except that there is no pair
of input entities.



관계 분류 작업의 경우 Soares 등(2019)이 제안한 Entity Markers-Entity Start 아키텍처60을 모델링에 사용한다. 여기서, 우리는 문장 내의 엔티티 범위를 몇 개의 특별한 단어 조각으로 둘러쌉니다. 첫 번째 엔티티와 두 번째 엔티티를 각각 [E1], [\E1] 및 [E2], [\E2]로 감싼다. 우리는 [E1]과 [E2]의 출력 임베딩을 연결한 다음 소프트맥스 활성화가 있는 선형 레이어를 통과한다. 우리는 언어 모델의 선형 계층 및 미세 조정 훈련을 위해 표준 교차 엔트로피 손실 함수를 사용한다.

기준선을 위해 Maini et al. (2020)50이 제안한 MaxPool과 MaxAt라는 두 가지 최신 모델을 사용한다. 이 접근법에서도 엔티티 쌍은 동일한 특수 토큰으로 포장된다. 그런 다음 입력 문장에 18개의 단어를 포함하는 글러브는 BiLSTM, 단어 위의 집계 메커니즘(MaxPool 및 MaxAt에 대해 다름) 및 소프트맥스 활성화가 있는 선형 계층을 통과한다.

종이 추상 분류 작업에서, 우리는 CLS 토큰의 출력 임베딩을 사용하여 전체 텍스트/추상을 인코딩한다. 우리는 예측을 하기 위해 이 임베딩을 간단한 분류기를 통해 전달한다. 우리는 트랜스포머 라이브러리 57의 BERT 문장 분류기 구현을 사용한다. 기준선의 경우 입력 엔티티 쌍이 없다는 점을 제외하고 관계 분류와 유사한 접근 방식을 사용한다.
