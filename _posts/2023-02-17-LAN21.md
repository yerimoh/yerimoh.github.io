...
* 그림 1에서 알 수 있듯이, adversarial training은 source style에 대한 에너지를 최대화하고 생성된 문장을 말뭉치의 분포에서 멀리 몰아내기 때문에, deceiving the discriminator를 하는 동안 fluency가 저하된다.    
* 유창성 저하에 대응하기 위해 언어 모델(LM)을 사용하여 생성된 문장을 말뭉치의 분포에 유지하는 정규화기를 도입한다.
➡ 이 LM-based regularizer는 문장을  target corpus로 끌어내어 생성된 문장을 말뭉치에 유지한다.
![image](https://user-images.githubusercontent.com/76824611/224559727-77c00525-9907-44c8-89f7-4027ccac0a2f.png)

Figure 1: Energy-based interpretation for fluency degradation. Deceiving energy-based discriminator D requires 1) minimizing the energy E between the transferred sentence xb′ and the target style s
′
, and 2) maximizing the energy between the sentence xb′ and the original
style s. However, the style s and s
′
are originated from
the same corpus, so maximizing E(xb′
, s) degrades the
