...
* [SCIBERT](https://yerimoh.github.io/LAN22/)  
...
<details>
<summary>📝 materials science domain에서의 NLP applications</summary>
<div markdown="1">
...
<details>
<summary>📜 pretraining 결과: Supplementary Fig1.</summary>
<div markdown="1">
...
<details>
<summary>📜 pretraining 이 모델 성능에 미치는 영향을 이해</summary>
<div markdown="1">
...
<details>
<summary>📜 longer durations train의 중요성</summary>
<div markdown="1">
...
<center>※ 각 table를 클릭하여 세부 사항을 확인하세요 ※</center> 

<details>
<summary>📝 Table 1. Macro-F1 scores on the test set for SOFC-Slot and SOFC datasets averaged over three seeds and five cross-validation splits</summary>
<div markdown="1">
...
* 이 측면을 추가로 평가하기 위해,     
Fig. 2 와 같이 SOFC-slot의 모든 17개 entity 유형에 대한 SciBERT와 MatSciBERT의 F1 점수를 개별적으로 분석했다.       
  * **모든 materials 관련 entity 유형**(anode material, cathode material, electrolytematerial, interlayer material, and support material)에서 **MatSciBERT가 SciBERT보다 성능이 우수**하다는 것을 관찰한다.    
...
<details>
<summary>📝 Table 2. Macro-F1 scores on the test set for Matscholar averaged over three seeds</summary>
<div markdown="1">
...
<details>
<summary>📝 Table 3. Test set results for Materials Synthesis Procedures dataset averaged over three seeds</summary>
<div markdown="1">
...
<details>
<summary>📝 Table 4. Test set results for glass vs. non-glass dataset averaged over three seeds</summary>
<div markdown="1">
...
<center>※ 각 번호를 클릭하여 세부 사항을 확인하세요 ※</center> 
...
이러한 이상 현상을 해결하기 위해 MSC의 Unicode normalization를 수행하여 다음을 수행했다:    
  ...
  * **2)** 비슷한 의미를 갖은 Unicode characters를 single standard character로 mapping해준다. ([실제 처리 보기](https://github.com/M3RG-IITD/MatSciBERT/blob/main/vocab_mappings.txt))   
...
**[MatSciBERT의 두가지 특징]**    
이 모델은 아래의 두가지 특징을 갖고 pre-train을 하였다.      
* MSC에서 MatSciBERT를 pre-train할 때 **SciBERT의 가중치로 MatSciBERT를 초기화**하고 SciBERT uncased vocabulary를 사용하여 토큰화를 수행    
* RoBERTa 사용      



**[train]**    
* MSC에서 256개 시퀀스의 배치 크기를 가진 2개의 NVIDIA V100 32GB GPU에서 15일 동안 최대 시퀀스 길이 512 토큰으로 MatSciBERT를 사전 교육한다.     
* AdamW 옵티마이저를 β1 = 0.9, β2 = 0.98, ε= $$1e^{-6}$$, weight decay = &&1e^{-2}&&로 사용        
* linear decay schedule for learning rate 를 warmup ratio = 4.8%, peak learning rate = $$1e^{−4}$$          
* PyTorch 및 Transformers 라이브러리를 사용하여 작성됨([GitHub repository](https://github.com/M3RG-IITD/MatSciBERT))      


<details>
<summary>📜 SciBERT의 가중치 사용</summary>
<div markdown="1"> 
 
R위에서 설명한대로 **MSC에서 MatSciBERT를 pre-train**한다.        
* LM을 처음부터  pre-train하려면 상당한 계산 능력과 대규모 데이터 세트가 필요하기 때문.    
* 이 문제를 해결하기 위해 **SciBERT의 가중치로 MatSciBERT를 초기화**하고 SciBERT uncased vocabulary를 사용하여 토큰화를 수행한다.         
* 이는 **생물의학 및 컴퓨터 과학 말뭉치**에 대해 사전 교육을 받은 SciBERT에 의존하는 기존 모델을 MatSciBERT와 **교환하여 사용**할 수 있다는 추가적인 이점이 있다.    

</div>
</details> 

<details>
<summary>📜 BERT를 수정한 RoBERTa</summary>
<div markdown="1"> 
 
RoBERTa는 BERT를 아래와 같이 수정한 것이다.    
구체적으로 MatSciBERT Pre-training을 위해 다음과 같은 간단한 수정이 채택되었다:    
* **Dynamic whole word masking:**    
BERT와 같이, wordpiece level(한 단어를 쪼갬)에서 마스킹하는 대신 **word level(단어단위로 봄)** 에서 masking.     
시퀀스가 샘플링될 때마다 무작위로 단어의 15%를 masking하고 모델이 각 마스크된 wordpiece level 토큰을 독립적으로 예측할 수 있도록 한다.      
* **Removing the NSP loss from the training objective:**            
BERT는 Masked-LM과 [Next sentence prediction(NSP)](https://yerimoh.github.io/Lan2/#1-pre-training-bert)이라는 두 가지  unsupervised을 사용하여 사전 훈련되었다.      
RoBERTa 저자는 **NSP loss을 제거**하면 다운스트림 작업 성능이 일치하거나 약간 향상된다고 주장한다.      
* **Training on full-length sequences:**    
BERT는 90%의 단계에 대해 128의 시퀀스 길이로, 나머지 10%의 단계에 대해 512의 시퀀스로 pre training했다.         
RoBERTa 저자들은 **전체 길이 시퀀스로만** 훈련함으로써 더 나은 성능을 얻었다.     
여기서 입력 시퀀스는 둘 이상의 문서의 세그먼트를 포함할 수 있으며 [SEP] 토큰은 입력 시퀀스 내에서 문서를 분리하는 데 사용된다     
*  **Using larger batch sizes:**    
더 큰 미니 배치를 사용한 훈련이 사전 훈련 손실을 개선하고 최종 작업 성능을 향상시킨다는 것을 발견했다
...
LM이 pre-trained된 후, 다양한 <span style="background-color:#fff5b1">**supervised downstream tasks에 finetune**</span>된다.     
* Pre-trained LM은 **task-specific output layer로 증강**된다.    
* 모델을 **specific tasks에 적응**시키고 **출력 계층에 존재하는 task-specific randomly initialized weights 를 학습하기 위해 Finetuning이 수행**된다.    
* **모든 end-to-end parameters에 대해 Finetuning이 수행**된다.  


**[3가지 downstream tasks NLP]**      
우리는 다음 **세 가지 downstream tasks NLP 작업**에서 MatSciBERT의 성능을 평가한다      


1️⃣ **Named Entity Recognition (NER)**     
주어진 문장에서 domain-specific named entities를 식별하는 것이다.     
**entities는** multi-token entities를 설명하기 위해 **[BIO 방식](https://wikidocs.net/24682)을 사용하여 인코딩**된다.         
NER 작업에 대한 데이터 세트에는 다양한 문장이 포함되며, **각 문장은 여러 토큰으로 분할**된다.        


BIO tagging은 아래와 같다.   
* **B**는 Begin의 약자로 개체명이 시작되는 부분   
* **I**는 Inside의 약자로 개체명의 내부 부분   
* **O**는 Outside로 개체명이 아니라는 의미    
...
2️⃣ **Input for the Relation Classification**     
이 task의,      
* input은, **문장과 해당 문장의 순서 있는 entities 쌍**으로 구성된다.      
* output은, 두 entities 간의 **directed relationship를 나타내는 label**입니다.        
* 두 entities 범위는 s1 = (i, j) 및 s2 = (k, l)로 표현될 수 있다,     
   * **i와 j:** 첫 번째 entities의 시작 및 종료 인덱스 (i ≤ j)     
   * **k와 l:** 입력 문에서 두 번째 entities의 시작 및 종료 인덱스 (k ≤ l), (j < k 또는 l < i)       
   * 제약조건은 두 entities의가 서로 겹치지 않아야하는 것이다.             
* output label은 L에 속하며, 여기서 L은 relation types의 fixed set이다.       
* 예시
이 작업은 **sentence과 entities 쌍이 입력**으로 주어지면 ```"Participant_Material"```, ```"Aparatus_Of"```와 같은 **레이블을 예측**하는 것이다.         
서로 다른 entities는 각각의 label과 함께 boxes가 쳐진다.   
관련 related entities는 relatio으로 레이블이 지정된 화살표를 사용하여 연결된다.
![image](https://user-images.githubusercontent.com/76824611/227092575-7f7007d8-7b0e-446d-b415-9fbcbc861683.png)



3️⃣ **Paper Abstract Classification task**     
우리는 연구 논문의 abstract을 받고, 우리는 그 abstract이 **주어진 분야와 관련이 있는지 아닌지를 분류**해야 한다.
...
다음 **세 가지 Materials Science-based NER NER 데이터 세트**를 사용하여 **SciBERT에 대한 MatSciBERT의 성능을 평가**한다:       

<center>※ 각 번호를 클릭하여 세부 사항을 확인하세요 ※</center>  


<details>
<summary>📝 1. Matscholar NER dataset</summary>
<div markdown="1"> 
 
[Matscholar NER dataset](https://pubmed.ncbi.nlm.nih.gov/31361962/)   

이 데이터 세트는 공개적으로 사용할 수 있으며 7가지 entities 유형을 포함한다.             
Training, validation, test set는 각각 440, 511, and 546 문장으로 이루어져있다.         
* inorganic material (MAT)       
* symmetry/phase label (SPL)     
* sample descriptor (DSC)      
* material property (PRO)       
* material application (APL)      
...
<details>
<summary>📝 2. Solid Oxide Fuel Cells – Entity Mention Extraction (SOFC) dataset</summary>
<div markdown="1"> 
 
[Solid Oxide Fuel Cells – Entity Mention Extraction (SOFC) dataset by Friedrich et al. (2020)](https://aclanthology.org/2020.acl-main.116/)
 

이 dataset는 도메인 전문가가 주석을 단 개방형 학술 논문 45편으로 구성된다.        

저자들은 아래 네 가지 유형의 entities에 주석을 달았다.     
Training, validation, test set는 각각 611개, 92개, 173개 문장으로 이루어져있다.      
* 재료(Material)    
* 실험(Experiment)     
* 가치(Value)   
* 장치(Device)  
...
<details>
<summary>📝 3. Solid Oxide Fuel Cells – Slot Filling (SOFC-Slot) dataset</summary>
<div markdown="1"> 
 
[Solid Oxide Fuel Cells – Slot Filling (SOFC-Slot) dataset by Friedrich et al. (2020)](https://aclanthology.org/2020.acl-main.116/)
 
**entity 유형이 더 세분화**되었다는 점을 제외하고는 위의 데이터 집합과 동일하다.    
16가지의 상이한 entity 타입이 존재한다.     
* 음극 재료(Anode Material)     
* 양극 재료(Cathode Material)     
* 전도도(Conductivity)    
* 전류 밀도(Current Density)    
* 열화율(Degradation Rate)    
* 소자(Device)      
* 전해질 재료(Electrolyte Material)     
* 연료 사용(Fuel Used)     
* 층간 재료(Interlayer Material)     
* 개방 회로 전압(Open Circuit Voltage)     
* 전력 밀도(Power Density)    
* 저항(Resistance)      
* 지지 재료(Support Material)        
* 작동 시간(Time of Operation)     
* 전압(Voltage)      
* 작동 온도(Working Temperature)      


두 가지 추가 entity 유형: Experiment Evoking Word 및 Thickness는 모델 train에 사용된다.      
...
relation classification를 위해 [Materials Synthesis Procedures dataset](https://doi.org/10.18653/v1/W19-4007)를 사용한다.       



<details>
<summary>📝 Materials Synthesis Procedures dataset</summary>
<div markdown="1"> 
 
 
**[Materials Synthesis Procedures dataset]**        
* **nodes**: synthesis steps의 participants의미     
* **edge**: node 간의 관계를 지정하는 그래프로 주석이 달린 230개의 synthesis steps로 구성된다.        
* synthesis procedure의 평균 길이는 9개의 문장(각 문장에는 평균 26개의 토큰이 존재).        
* 데이터 세트는 16개의 **relation labels**로 구성된다.       


**relation labels**은 저자에 의해 **three categories**로 분류되었다:           
* a. **Operation-Argument relations:**       
     * Recipe target        
     * Solvent material     
     ...
     * Apparatus of     
     * Condition of          
* b. **Non-Operation Entity relations:**      
     * 설명자(Descriptor of)    
     * 숫자(Number of)    
     * 수량(Amount of)     
     * 장치 특성(Apparatus-attr-of)    
     * 브랜드(Brand of)    
     * 핵심(Core of)     
     * 속성(Property of)   
     * 유형( Type of)   
* c. **Operation-Operation relations:**      
     * Next operation(다음 작업)


Training, validation, test set는 각각 150, 30, 50개의 주석이 달린 material synthesis로 구성된다       
...
NER task의 경우,    
모든 token의 첫 번째 wordpiece의 **BERT contextual output embedding**을 사용하여 **|L| 클래스 간에 토큰을 분류**한다.     
LM-Linear, LM-CRF 및 LMbiLSTM-CRF의 세 가지 아키텍처를 사용하여 NER 작업을 모델링한다.      
여기서 LM은 모든 BERT-based transformer로 대체될 수 있다.       
우리는 이 작업에서 LM을 BERT, SciBERT 및 MatSciBERT로 간주한다.     


* **1. LM-Linear:**           
wordpieces의 출력 임베딩은 softmax activation과 함께 linear layer를 통과한다.     
우리는 [transformers library](https://arxiv.org/pdf/1912.01703.pdf)의 BERT Token Classifier 구현을 사용한다.    
* **2. LM-CRF:**     
모델이 언급된 동일한 entity에 속하는 토큰에 레이블을 지정하고 다른 entity 유형 간의 transition scores를 학습할 수 있도록 LM-Linear architecture의 final softmax activation를 [CRF layer](https://dl.acm.org/doi/10.5555/645530.655813)로 대체한다.            
우리는 [PyTorch-CRF 라이브러리](https://pytorch-crf.readthedocs.io/en/stable/)의 CRF 구현을 사용한다.     
* **LM-BiLSTM-CRF**     
[Bidirectional Long Short-Term Memory](https://arxiv.org/abs/1508.01991)은 LM과 CRF 층 사이에 추가된다.     
모든 wordpieces의 BERT 임베딩은 stacked BiLSTM을 통해 전달된다.      
BiLSTM의 출력은 예측을 하기 위해 마지막으로 CRF layer에 공급된다.      
...
Relation Classification task의 경우 [Entity Markers-Entity Start 아키텍처](https://aclanthology.org/P19-1279/)을 모델링에 사용한다.      
* 여기서, 우리는 문장 내의 entity 범위를 몇 개의 **special wordpieces**으로 둘러싼다.       
첫 번째 엔티티와 두 번째 엔티티를 각각 ```[E1]```, ```[\E1]``` 및 ```[E2]```, ```[\E2]```로 감싼다.     
우리는 ```[E1]```과 ```[E2]```의 출력 임베딩을 연결한 다음 softmax activation가 있는 선형 레이어를 통과한다.    
우리는 언어 모델의 linear layer 및 finetuning 훈련을 위해 standard cross-entropy loss function를 사용한다.    
* baseline을 위해 [**MaxPool과 MaxAtt**](https://aclanthology.org/2020.findings-emnlp.410/)라는 두 가지 최신 모델을 사용한다.      
이 접근법에서도 엔티티 쌍은 same special tokens으로 포장된다.      
그런 다음 [GloVe embeddings](https://aclanthology.org/D14-1162/)을 통과한 input sentence는 BiLSTM을 통과하고,    
agregation mechanism(MaxPool 및 MaxAtt에 대해 다름) 및 softmax activation가 있는 linear layer을 통과한다.    
* Paper Abstract Classification task에서, 우리는 **```CLS``` 토큰의 output embedding을 사용하여  entire text/abstract을 인코딩**한다.     
우리는 예측을 하기 위해 이 임베딩을 간단한 분류기를 통해 전달한다.     
우리는 BERT Sentence Classifier implementation을 사용한다.    
baseline의 경우 입력 엔티티 쌍이 없다는 점을 제외하고 관계 분류와 유사한 접근 방식을 사용한다.    
...
* [Evaluation metrics](https://github.com/spyysalo/conlleval.py)        
* [DATA AVAILABILITY](https://github.com/M3RG-IITD/MatSciBERT/blob/main/pretraining/piis_dois.csv)     
* CODE AVAILABILITY     
   * [All the codes used in the present work](https://github.com/M3RG-IITD/MatSciBERT)       
   * [codes with finetuned models for the downstream tasks](https://zenodo.org/record/6413296#.ZByBaXZBy5c)    
