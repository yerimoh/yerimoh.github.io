...
simple topic-based classification of research papers to discovering
...
classification: A large number of manuscripts have been published
...
classification algorithm. However, these approaches directly
vectorize a word and are not context sensitive. For instance, in
the phrases “flat glass”, “glass transition temperature”, “tea glass”,
...
thereby enabling improved topic classification. This is evident
from the binary classification results presented earlier in Table 4,
...
modeling using LDA. Specifically, we obtain the output embeddings of the [CLS] token for each abstract using MatSciBERT.
Further, these embeddings were projected into two dimensions
using the UMAP algorithm51 and then clustered using the
k-means algorithm52. We then concatenate all the abstracts
...
table. We observe that the topics and keywords from MatSciBERTbased topic modeling are more coherent than the ones obtained
from LDA. Further, the actual topics associated with the keywords
are not very apparent from Supplementary Table 5. Specifically,
...
some of the topics identified based on the keywords by three
selected domain experts are dissolution of silicates (9), oxide thin
films synthesis and their properties (8, 6), materials for energy (0),
...
finetuned on NER can be an extremely useful tool for extracting
information from figure captions.
