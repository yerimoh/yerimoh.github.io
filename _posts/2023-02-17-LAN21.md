...
## Downstream tasks
Once the LM is pre-trained, we finetune it on various supervised
downstream tasks. Pre-trained LM is augmented with a task-specific
output layer. Finetuning is done to adapt the model to specific tasks as
well as to learn the task-specific randomly initialized weights present in the
output layer. Finetuning is done on all the parameters end-to-end. We
evaluate the performance of MatSciBERT on the following three downstream NLP tasks:
...
2. Input for the Relation Classification     

이 task의,      
* input은, **문장과 해당 문장의 순서 있는 entities 쌍**으로 구성된다.      
* output은, 두 entities 간의 directed relationship를 나타내는 label입니다. 두 엔티티 범위는 s1 = (i, j) 및 s2 = (k, l)로 표현될 수 있으며, 여기서 i와 j는 첫 번째 엔티티의 시작 및 종료 인덱스를 나타내고 k와 l은 입력 문에서 두 번째 엔티티의 시작 및 종료 인덱스를 나타냅니다. 여기서, i ≤ j, k ≤ l 및 (j < k 또는 l < i). 마지막 제약조건은 두 엔티티가 서로 겹치지 않도록 보장한다. 출력 레이블은 L에 속하며, 여기서 L은 관계 유형의 고정 집합입니다. 과제에서 나온 문장의 예는 그림 6에 나와 있다. 이 작업은 문장과 엔티티 쌍이 입력으로 주어지면 "Participant_Material", "Aparatus_Of"와 같은 레이블을 예측하는 것이다.

논문 초록 분류 과제에서, 우리는 연구 논문의 초록을 받고, 우리는 그 초록이 주어진 분야와 관련이 있는지 아닌지를 분류해야 한다
1. Named Entity Recognition (NER) involves identifying domainspecific named entities in a given sentence. Entities are encoded
using the BIO scheme to account for multi-token entities53. Dataset
...
yn] are labels for these tokens, then each yi ∈ L = {B-e1, I-e1, … B-ek,
I-ek, O}. Here, B-ei and I-ei represent the beginning and inside of
entity ei


2. Input for the Relation Classification60 task consists of a sentence and
...
with each other. The output label belongs to L, where L is a fixed set
...
relevant to a given field or not.
