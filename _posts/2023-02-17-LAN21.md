...
<span style="background-color:#F5F5F5">**[Fig. 2: Comparison of MatSciBERT and SciBERT on validation sets
of SOFC-Slot dataset]**</span>      
...
MatSciBERT의 성능을 입증하기 위해,    
 우리는 Supplementary Figs. 3 and 4에서 **validation set** of the dataset의 예를 보여준다.     
...
In Supplementary Figure 3, we show a sentence from a manuscript related to alloys. The
entities are colored based on their true labels. From the predictions, it was observed that both
MatSciBERT and SciBERT misclassified the phrase ‘dislocation cell-like structure’ as ‘PRO’,
which was labeled as ‘O’ or ‘Other’ in the dataset. Further, ‘ND’ and ‘TD’ were labeled as
‘PRO’ by SciBERT, but MatSciBERT was able to correctly identify these entities as ‘O’.
Overall, we observe that MatSciBERT is able to identify the context in the text related to
materials, from which the entities are correctly tagged. The mistakes made by SciBERT are
written in normal font (non-italicized) and the mistakes made by MatSciBERT are underlined.
Similarly, Supplementary Figure 4 shows the result obtained using SciBERT, where it has
labeled ‘YSZ’ (also called yttria-stabilized zirconia) as B-electrolyte_material that has a true
label of B-interlayer_material. However, MatSciBERT was able to correctly identify the label
of ‘YSZ’. Note that these are some arbitrary examples selected to demonstrate the performances
of both SciBERT and MatSciBERT.
Ground truth and model pred
...
표 3은 재료 합성 절차 데이터 세트 46에서 수행된 관계 분류 작업의 결과를 보여줍니다. 우리는 또한 그 결과를 두 개의 최신 기준 모델인 MaxPool과 MaxAt50과 비교하는데, 이에 대한 자세한 내용은 방법 섹션에서 확인할 수 있다. 이 작업에서도 MatSciBERT는 마진이 낮지만 SciBERT, BERT 및 기준 모델보다 일관되게 성능이 우수합니다.

종이 추상 분류 다운스트림 작업에서, 우리는 LMs가 사내 데이터 세트 10을 기반으로 원고를 유리 주제 대 비유리 주제로 분류할 수 있는 능력을 고려한다. 이것은 입력이 원고의 추상인 이진 분류 문제이다. 여기서도 동일한 기준선 모델인 MaxPool과 MaxAt50을 사용합니다. 표 4는 MatSciBERT, SciBERT, BERT 및 기준선에 의해 달성된 정확도의 비교를 보여준다. MatSciBERT는 테스트 세트에서 SciBERT를 2.75% 이상 정확도로 능가한다는 것을 분명히 알 수 있다.

종합적으로, 우리는 재료 과학 말뭉치에 대해 사전 훈련된 MatSciBERT가 재료 데이터 세트에 대한 NER, 추상 분류 및 관계 분류와 같은 모든 다운스트림 작업에서 SciBERT보다 더 나은 성능을 발휘할 수 있음을 보여준다. 이러한 결과는 또한 MatSciBERT가 사전 훈련된 재료 영역의 과학 문헌이 SciBERT가 있는 컴퓨터 과학 및 생물 의학 영역과 크게 다르다는 것을 시사한다

훈련된. 특히, 각 과학 분야는 온톨로지, 어휘 및 도메인별 표기법 측면에서 상당한 가변성을 보인다. 따라서 과학 문헌 내에서도 도메인별 언어 모델의 개발은 문헌에서 텍스트 마이닝 및 정보 추출과 관련된 다운스트림 작업의 성능을 크게 향상시킬 수 있다
