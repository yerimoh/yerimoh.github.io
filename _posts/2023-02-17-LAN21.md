...
위에서 설명한대로 **MSC에서 MatSciBERT를 pre-train**한다.        
...
**[MatSciBERT pretrain: RoBERTa 사용]**           
...
훈련 목표에서 NSP 손실 제거: BERT는 Masked-LM과 다음 문장 예측(NSP)이라는 두 가지 비지도 작업을 사용하여 사전 훈련되었다. NSP는 한 쌍의 문장을 입력하고 두 문장이 서로 따르는지 여부를 예측합니다. RoBERTa 저자는 NSP 손실을 제거하면 다운스트림 작업 성능이 일치하거나 약간 향상된다고 주장한다.

전체 길이 시퀀스에 대한 교육: BERT는 90%의 단계에 대해 128의 시퀀스 길이로, 나머지 10%의 단계에 대해 512의 시퀀스로 사전 교육을 받았다. RoBERTa 저자들은 전체 길이 시퀀스로만 훈련함으로써 더 나은 성능을 얻었다. 여기서 입력 시퀀스는 둘 이상의 문서의 세그먼트를 포함할 수 있으며 [SEP] 토큰은 입력 시퀀스 내에서 문서를 분리하는 데 사용됩니다.

더 큰 배치 크기 사용: 저자들은 또한 더 큰 미니 배치를 사용한 훈련이 사전 훈련 손실을 개선하고 최종 작업 성능을 향상시킨다는 것을 발견했다

이러한 수정 사항에 따라, 우리는 MSC에서 256개 시퀀스의 배치 크기를 가진 2개의 NVIDIA V100 32GB GPU에서 15일 동안 최대 시퀀스 길이 512 토큰으로 MatSciBERT를 사전 교육한다. 우리는 AdamW 옵티마이저를 β1 = 0.9, β2 = 0.98, β= 1e-6, 가중치 붕괴 = 1e-2 및 최대 학습률 = 1e-4와 함께 사용한다. 사전 교육 코드는 PyTorch59 및 Transformers57 라이브러리를 사용하여 작성되었으며 이 작업을 위해 GithHub 저장소에서 사용할 수 있다 https://github.com/M3RG-IITD/MatSciBERT .
