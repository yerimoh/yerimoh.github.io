...
관계 분류를 위해 Mysore et al. (2019)46의 재료 합성 절차 데이터 세트를 사용한다. 이 데이터 세트는 노드가 합성 단계의 참가자를 나타내며 에지가 노드 간의 관계를 지정하는 그래프로 주석이 달린 230개의 합성 절차로 구성된다. 합성 과정의 평균 길이는 9개의 문장이며, 각 문장에는 평균 26개의 토큰이 존재한다. 데이터 세트는 16개의 관계 레이블로 구성됩니다. 관계 라벨은 저자에 의해 세 가지 범주로 분류되었다:


For relation classification, we use the Materials Synthesis Procedures
dataset by Mysore et al. (2019)46. This dataset consists of 230 synthesis
...
a. 작업-인수 관계: Recipe Target, Solvent material, 분위기 material, Recipe precipe precipe, 참가자 material, 장비, 조건

b. 비운영 주체 관계: 설명자, 숫자, 수량, 장치 특성, 브랜드, 핵심, 속성, 유형

c. 작업-작업 관계: 다음 작업

훈련, 검증 및 테스트 세트는 각각 150, 30, 50개의 주석이 달린 재료 합성 절차로 구성된다

추상화를 기반으로 유리 과학과 관련된 연구 논문을 분류하기 위한 데이터 세트는 Venugopal et al. (2021)10에서 가져왔다. 저자들은 1500개의 추상적인 것들을 유리와 유리가 아닌 것으로 수동으로 분류했다. 이러한 추상체들은 생물 활성 안경, 희토류 안경, 유리 세라믹, 박막 연구, 안경의 광학, 유전체 및 열적 특성과 같은 다양한 유리 과학 분야에 속한다. 우리는 추상화를 3:1:1의 열차-검증-테스트 분할로 나눈다.


a. Operation-Argument relations: Recipe target, Solvent material,
Atmospheric material, Recipe precursor, Participant material, Apparatus of, Condition of
...
(2021)10. The authors have manually labeled 1500 abstracts as glass and
non-glass. These abstracts belong to different fields of glass science like
bioactive glasses, rare-earth glasses, glass ceramics, thin-film studies, and
...
## Modeling
For NER task, we use the BERT contextual output embedding of the first
wordpiece of every token to classify the tokens among |L | classes. We
model the NER task using three architectures: LM-Linear, LM-CRF, and LMBiLSTM-CRF. Here, LM can be replaced by any BERT-based transformer
