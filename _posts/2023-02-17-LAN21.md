...
명명된 엔티티 인식(NER)은 주어진 문장에서 도메인 특정 명명된 엔티티를 식별하는 것을 포함한다. 엔티티는 다중 토큰 엔티티를 설명하기 위해 BIO 방식을 사용하여 인코딩된다. NER 작업에 대한 데이터 세트에는 다양한 문장이 포함되며, 각 문장은 여러 토큰으로 분할된다. 각 토큰에 대해 골드 레이블이 제공됩니다. 좀 더 공식적으로, E = {e1, …ek}를 주어진 데이터 집합에 대한 엔티티 유형의 집합이라고 하자. 만약 [x1, …xn]이 문장의 토큰이고 [y1, …yn]이 이러한 토큰에 대한 레이블이라면, 각 y∈ L = {B-e1, I-e1, … B-ek, O}이다. 여기서 B-ei와 I-ei는 엔티티 ei의 시작과 내부를 나타냅니다
