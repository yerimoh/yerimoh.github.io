...
LM이 pre-trained되면 다양한 supervised downstream tasks에 finetune된다.     
Pre-trained LM은 task-specific output layer로 증강된다.    
모델을 specific tasks에 적응시키고 출력 계층에 존재하는 task-specific randomly initialized weights 를 학습하기 위해 Finetuning이 수행된다.    
모든 end-to-end parameters에 대해 Finetuning이 수행된다. 우리는 다음 세 가지 다운스트림 NLP 작업에서 MatSciBERT의 성능을 평가한다:

1. 명명된 엔티티 인식(NER)은 주어진 문장에서 도메인 특정 명명된 엔티티를 식별하는 것을 포함한다. 엔티티는 다중 토큰 엔티티를 설명하기 위해 BIO 방식을 사용하여 인코딩된다. NER 작업에 대한 데이터 세트에는 다양한 문장이 포함되며, 각 문장은 여러 토큰으로 분할된다. 각 토큰에 대해 골드 레이블이 제공됩니다. 좀 더 공식적으로, E = {e1, …ek}를 주어진 데이터 집합에 대한 엔티티 유형의 집합이라고 하자. 만약 [x1, …xn]이 문장의 토큰이고 [y1, …yn]이 이러한 토큰에 대한 레이블이라면, 각 y∈ L = {B-e1, I-e1, … B-ek, O}이다. 여기서 B-ei와 I-ei는 엔티티 ei의 시작과 내부를 나타냅니다

관계 분류 60 작업에 대한 입력은 문장과 해당 문장의 순서 있는 엔티티 쌍으로 구성된다. 출력은 두 엔티티 간의 지시된 관계를 나타내는 레이블입니다. 두 엔티티 범위는 s1 = (i, j) 및 s2 = (k, l)로 표현될 수 있으며, 여기서 i와 j는 첫 번째 엔티티의 시작 및 종료 인덱스를 나타내고 k와 l은 입력 문에서 두 번째 엔티티의 시작 및 종료 인덱스를 나타냅니다. 여기서, i ≤ j, k ≤ l 및 (j < k 또는 l < i). 마지막 제약조건은 두 엔티티가 서로 겹치지 않도록 보장한다. 출력 레이블은 L에 속하며, 여기서 L은 관계 유형의 고정 집합입니다. 과제에서 나온 문장의 예는 그림 6에 나와 있다. 이 작업은 문장과 엔티티 쌍이 입력으로 주어지면 "Participant_Material", "Aparatus_Of"와 같은 레이블을 예측하는 것이다.
