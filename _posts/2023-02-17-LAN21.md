...
우리는  Gururangan et al. (2020) (20)이 제안한 domain-specific 적응 pre-training을 따른다.      
이 연구에서 저자들은 도메인별 텍스트 20의 말뭉치에 대한 초기 LM의 사전 훈련을 계속했다. 그들은 초기 LM 어휘와 도메인별 어휘 사이의 중복이 54.1% 미만임에도 불구하고 4개 도메인 모두에 대한 도메인별 다운스트림 작업의 성능이 크게 향상되는 것을 관찰했다. BioBERT19와 FinBERT22는 바닐라 BERT 모델이 도메인별 텍스트에 대해 추가로 사전 훈련된 유사한 접근 방식을 사용하여 개발되었으며 토큰화는 원래 BERT 어휘를 사용하여 수행된다. 우리는 MatSciBERT 가중치를 일부 적합한 LM의 가중치로 초기화한 다음 MSC에서 사전 훈련한다. MatSciBERT에 대한 적절한 초기 가중치를 결정하기 위해, 우리는 토큰화자 라이브러리 48을 사용하여 MSC를 기반으로 케이스 없는 단어 조각 47 어휘를 훈련시켰다. MSC 어휘의 중복은 사례가 없는 SciBERT21 어휘의 경우 53.64%, 사례가 없는 BERT 어휘의 경우 38.90%이다. SciBERT의 어휘와 더 큰 중복으로 인해, 우리는 SciBERT 어휘를 사용하여 말뭉치를 토큰화하고 Beltagy 등(2019)21에 의해 공개된 SciBERT의 가중치로 MatSciBERT 가중치를 초기화한다. 재료 과학 분야별 어휘는 말뭉치를 더 적은 수의 단어로 표현하고 잠재적으로 더 나은 언어 모델로 이어질 수 있다는 것을 언급할 가치가 있다. 예를 들어, 이트리아 안정화 지르코니아는 SciBERT 어휘에 의해 [yt], "##tri", "#a", "-", "stabilized", "zircon", "##ia"]로 토큰화되는 반면, 도메인 특이적 토큰화는 [yttria", "-", "stabilized", "zircon"로 결과를 초래할 수 있다. 그러나 도메인별 토큰화기를 사용하면 SciBERT 가중치를 사용할 수 없으며 SciBERT가 이미 학습한 과학 지식을 활용한다. 또한 심층 신경 언어 모델은 기존 토큰화기를 사용하여 새로운 단어를 나타내는 반복 패턴을 학습할 수 있기 때문에 재료 영역에 SciBERT 어휘를 사용하는 것이 반드시 해로운 것은 아니다. 예를 들어, 단어 조각 "yt", "##tri", 그리고 "##a"가 연속적으로 발생할 때 SciBERT는 다운스트림 작업에서 입증된 것처럼 어떤 물질이 논의되고 있음을 인식한다, 및 ClinicalBERT40은 도메인별 토큰화자를 사용하고 처음부터 학습하는 대신 사전 교육을 확장한다.
...
사전 교육 절차에 대한 자세한 내용은 방법 섹션에 나와 있습니다. 사전 훈련은 360시간 동안 수행되었고, 그 후 모델은 검증 세트에서 2.998의 최종 혼란을 달성했다(보조 그림 1a 참조). 서로 다른 어휘와 검증 말뭉치로 인해 직접 비교할 수는 없지만, BERT25와 RoBERTa49 저자는 각각 동일한 범위에 있는 3.99와 3.68로 혼동을 보고한다. 우리는 또한 보충 그림 1b, c의 MLM 손실 및 MLM 정확도와 같은 다른 평가 지표에 대한 그래프를 제공한다. 그런 다음 최종 사전 훈련된 LM을 사용하여 다양한 재료 과학 영역별 다운스트림 작업을 평가했으며, 이에 대한 자세한 내용은 후속 섹션에 설명되어 있다. 다운스트림 작업에서 LM의 성능을 SciBERT, BERT 및 기타 기준 모델과 비교하여 재료의 특정 정보를 학습하기 위해 MatSciBERT의 효과를 평가했다.
...
사전 훈련이 모델 성능에 미치는 영향을 이해하기 위해, SOFC-슬롯에 대한 재료 도메인별 다운스트림 작업인 NER이 사전 훈련의 정기적인 간격으로 모델을 사용하여 수행되었다. 이 정도로, 사전 훈련된 모델은 SOFC 슬롯 데이터 세트의 훈련 세트에서 미세 조정되었다. SOFC 슬롯 데이터 세트의 선택은 데이터 세트가 세분화된 재료별 정보로 구성되어 있다는 사실에 기초했다. 따라서 이 데이터 세트는 SciBERT의 성능을 재료 인식 LM과 구별하는 데 적합하다. 이러한 미세 조정된 모델의 성능은 테스트 세트에서 평가되었다. LM-CRF 아키텍처는 이 연구의 후반부에서 보여주듯이 다운스트림 작업에 대해 지속적으로 최상의 성능을 제공하기 때문에 분석에 사용되었다. 세 씨앗에 걸친 매크로-F1 평균은 증가 추세를 보여(보조 그림 2a 참조), 더 긴 기간에 대한 훈련의 중요성을 시사했다. 우리는 또한 추상 분류 작업에 대한 유사한 그래프를 보여준다(보조 그림 2b).
