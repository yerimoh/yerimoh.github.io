...
BookCorpus과 영어와 같은 많은 범용 말뭉치들이 있다위키피디아, 그리고 생물 의학 코퍼스 39, 임상 데이터베이스 41과 같은 도메인별 말뭉치. 그러나 이러한 말뭉치 중 어느 것도 재료 영역에 적합하지 않습니다. 따라서 재료별 LM을 제공하기 위해 먼저 무기 유리, 금속 유리, 합금, 시멘트 및 콘크리트의 네 가지 중요한 재료 과학 제품군에 걸친 말뭉치를 생성한다. 이러한 광범위한 범주가 언급되었지만, 2차원 물질을 포함한 몇 가지 다른 범주의 물질도 말뭉치에 존재했다는 점에 유의해야 한다. 구체적으로, 우리는 엘스비어 사이언스 다이렉트 데이터베이스에서 다운로드한 약 1백만 개의 논문 중에서 약 150만 개의 논문을 선택했다. 말뭉치를 만드는 단계는 방법 섹션에 나와 있습니다. 각 가정의 논문 수와 단어 수에 대한 자세한 내용은 보충표 1에 나와 있다. 우리는 또한 이 작업을 위해 GitHub 저장소에서 MatSciBERT를 사전 훈련하는 데 사용되는 논문의 DOI와 PII 목록을 제공했다

이 연구를 위해 개발된 재료 과학 말뭉치는 ~285M 단어를 가지고 있으며, 이는 SciBERT(3.17B 단어)와 BERT(3.3B 단어)를 사전 훈련하는 데 사용되는 단어의 거의 9%이다. SciBERT 사전 훈련을 계속하기 때문에 MatSciBERT는 3.17 + 0.28 = 3.45B 단어로 구성된 말뭉치에 대해 효과적으로 훈련된다. 보충표 1로부터, 단어의 40%는 무기물 유리 및 세라믹과 관련된 연구 논문에서, 각각 20%는 벌크 금속 유리(BMG), 합금 및 시멘트에서 나온 것을 알 수 있다. '시멘트와 콘크리트'의 연구 논문 수는 '무기 유리와 도자기'보다 많지만, 후자가 단어 수가 더 많다. 이는 후자의 범주와 관련하여 검색된 많은 수의 전체 텍스트 문서가 존재하기 때문입니다. 보충표 2는 재료과학 분야와 관련된 중요한 문자열의 단어 수를 나타낸다. 말뭉치는 열전, 나노 재료, 고분자 및 생체 재료의 중요한 분야를 포괄한다는 점에 유의해야 한다. 또한 언어 모델을 훈련하는 데 사용되는 말뭉치는 실험 및 계산 작업으로 구성되어 있으며, 이 두 가지 접근 방식 모두 물질 반응을 이해하는 데 중요한 역할을 한다. 이 말뭉치의 평균 종이 길이는 ~1848 단어이며, 이는 SciBERT 말뭉치의 평균 종이 길이 2769 단어의 3분의 2이다. 평균 논문 길이가 낮은 것은 다음 두 가지에 기인할 수 있다: (a) 일반적으로 재료 과학 논문은 생물 의학 논문보다 짧다. 우리는 전문 자료 과학 논문의 평균 논문 길이를 계산하여 이를 검증했다. 2366명으로 나왔다. (b) 우리 말뭉치에도 전문이 없는 논문이 있다. 그러한 경우, 우리는 최종 말뭉치에 도달하기 위해 그러한 논문의 요약을 사용했다.
