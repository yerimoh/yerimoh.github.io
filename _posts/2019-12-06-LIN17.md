...
$$
\text{벡터공간 } V \text{의 어떤 부분집합 } S \subseteq V \text{ 에 대해,}
$$

$$
S \text{ 안의 일차 독립인 벡터들 중, 더 이상 벡터를 추가하면 선형 독립이 깨지는 최대한 큰 부분집합을 }
$$

**일차 독립인 극대 부분집합** (maximal linearly independent subset)이라 한다.

즉,

$$
L \subseteq S \text{ 가 선형독립이고, } L \cup \{v\} \text{ 가 선형독립이 되도록 할 수 있는 } v \in S \setminus L \text{ 는 존재하지 않는다.}
$$
