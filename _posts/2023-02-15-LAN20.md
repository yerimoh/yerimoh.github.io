...
초기 연구에서는 소스 도메인 정보를 주입하고 도메인 분류기를 추가하는 두 가지 주요 접근 방식이 있었다.     
소스 도메인 정보를 추가하기 위해, Kobus 등(2017)은 소스 도메인 레이블을 입력이 있는 추가 태그 또는 보완 기능으로 삽입한다. 두 번째 접근법을 위해 Britz 등(2017)은 도메인 분류기의 그레이디언트를 사용하여 도메인별 문장 임베딩을 훈련한다
