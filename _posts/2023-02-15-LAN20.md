...
, 여기서 y<i는 yi를 제외한 최대 값의 대상 하위 단어이다. 간단하게 하기 위해, 우리는 Eq. (3)를 XMI(i)로 표기할 것이다. 낮은 XMI(i)는 우리의 도메인 적응 모델이 번역 중에 도메인 정보를 완전히 활용하지 않는다는 것을 의미한다. 따라서, 우리는 낮은 XMI(i)를 가진 토큰에 더 많은 가중치를 부여하여 Eq를 최소화한다,
...
, 여기서 nT는 대상 문장에서 하위 단어 토큰의 수입니다.

우리 방법의 최종 손실은 Eq. (7)에 있으며, 여기서 φ1과 φ2는 초 매개 변수이다
