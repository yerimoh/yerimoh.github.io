...
BoW에 기반한 [통계기반 분석](https://yerimoh.github.io/DL13/)은 단어의 **빈도** 수를 이용한 통계적 분석이기 때문에 **단어의 의미**를 고려하지 못한다.      
...
즉 각 원소의 중요도가 낮은 것이다.    
이러한 벡터는 누가 들어도 비효율적이고 비현실적이며 노이즈에 약하고 견고하지 못하다.      
...
* **$$U$$**: NxN 정방행렬로 직교행렬을 만족해야한다.          
$$U∈R^{N×N}$$    

* **$$V$$**: MxM 정방행렬로 직교행렬을 만족해야한다.          
$$V∈R^{MxM}$$    
...
* **$$Σ$$**: NxM의 대각성분이 양수인 대각행렬이어야 한다. 큰 수부터 작은 수 순서로 배열한다.   
